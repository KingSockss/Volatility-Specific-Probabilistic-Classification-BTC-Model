Model B is a probabilistic BTC spot model that generates minutely YES-probability forecasts for the hourly Kalshi BTC contracts defined in `Data_Sourcing/Settlement_Outcomes/kalshi_btc_atm_settlements.csv`.

Pipeline

1. Use the settlement CSV as the contract manifest, but restrict the event universe to the hourly Kalshi pricing files so Model B does not generate past the Model K horizon.
...
9. Write one hourly CSV per event under `Model_B_Output_Raw_Data` using the same raw layout as `Data_Sourcing/Kalshi_Pricing_Fetch/hourly_events_price_data`.
...
`Model_B/Model_B_Output_Raw_Data`
...
- The default implementation re-fits parameters every 60 forecast minutes, uses a trailing 90-day regime-selection window, and trains on the `high_volatility` segment defined by the existing 72-hour rolling percentile rules.
- Model B separates parameter estimation from state initialization: regime-filtered returns drive the MLE fit, while the contiguous `--history-minutes` window drives the live conditional-variance seed used for simulation.
