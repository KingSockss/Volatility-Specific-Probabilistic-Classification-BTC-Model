...
ASSUMPTIONS = (
    "Model N forecast values are interpreted as YES probabilities. CSV values greater than 1 and no greater than 100 "
    "are treated as cents and divided by 100 before scoring.",
    "Model N uses p_final = (1 - q_shock) * p_normal + q_shock * p_shock.",
    "Model N also carries p_exponential_hybrid with lambda=2 as a downstream strategy probability: "
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate Model N hybrid BTC hourly contract probabilities against official Kalshi outcomes."
    )
    parser.add_argument(
        "--kalshi-price-dir",
        type=Path,
        default=root / "Model_N" / "Model_N_Output_Raw_Data",
        help="Folder containing Model_N.py hourly raw forecast CSV outputs.",
    )
    parser.add_argument(
        "--settlement-csv",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "model_N_Evals_Outputs",
        help="Directory for Model N evaluation output files.",
    )
...
def load_model_n_price_outputs(price_dir: Path) -> pd.DataFrame:
...
def write_individual_metric_files(
    *,
    ...
    (output_dir / "model_n_assumptions.txt").write_text("\n".join(ASSUMPTIONS) + "\n", encoding="utf-8")


def model_n_html(html_text: str) -> str:
    replacements = {
        "Model R": "Model N",
        "model_R_Evals_Outputs": "model_N_Evals_Outputs",
        "model_r_assumptions.txt": "model_n_assumptions.txt",
        "Model_R": "Model_N",
    }
    out = html_text
...
def main() -> None:
    ...
    forecasts = load_model_n_price_outputs(args.kalshi_price_dir.resolve())
    ...
    (output_dir / "model_n_summary.html").write_text(model_n_html(summary_html), encoding="utf-8")
    (output_dir / "model_n_summary_with_coverage.html").write_text(
        model_n_html(summary_with_coverage_html),
        encoding="utf-8",
    )
    ...
    print(f"Model N evaluation output directory: {output_dir}")
