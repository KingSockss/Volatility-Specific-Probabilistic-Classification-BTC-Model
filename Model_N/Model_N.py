...
RAW_OUTPUT_FOLDER_NAME = "Model_N_Output_Raw_Data"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build Model N hybrid raw forecasts. The default Model N price remains the "
    ...
    parser.add_argument(
        ...
        help="Model R raw output directory, used as Model N model_a / p_normal.",
    )
    parser.add_argument(
        "--shock-raw-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=root / "Model_N" / "Additional_Data" / "all_test_predictions.csv",
    ...
    parser.add_argument(
        ...
        help="Directory where Model N per-hour raw forecast CSV files will be written.",
    )
...
def main() -> None:
    ...
    if not files_written:
        raise RuntimeError("No Model N raw files were written; check q_shock/source overlap.")
    ...
    print(f"Model N q_shock source: {args.shock_probability_csv.resolve()}")
    print(f"Model N output directory: {output_dir}")
