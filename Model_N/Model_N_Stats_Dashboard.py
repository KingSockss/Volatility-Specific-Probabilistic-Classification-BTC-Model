...
OUTPUT_FOLDER_NAME = "Model_N_Stats_Dashboard_Outputs"


def default_model_n_eval_dir(root: Path) -> Path:
    return root / "Model_N" / "model_N_Evals_Outputs"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build internal and comparative dashboards for Model N using evaluation output folders."
    )
    parser.add_argument("--model-n-eval-dir", type=Path, default=default_model_n_eval_dir(base.REPO_ROOT))
...
def main() -> None:
    ...
    model_n_eval_dir = args.model_n_eval_dir.resolve()
    model_k_eval_dir = args.model_k_eval_dir.resolve()

    loaded_n = base.load_eval_dir(model_n_eval_dir)
    ...
    internal_notes = [
        f"Source directory: {model_n_eval_dir}",
        f"Randomized PIT seed: {args.pit_seed}",
        "Internal dashboard uses the full Model N scored universe currently present in raw_values.csv.",
    ]
    internal_html = base.build_internal_dashboard_html(bundle_n_internal, pit_values, internal_notes)
    (output_dir / "model_n_internal_dashboard.html").write_text(model_n_html(internal_html), encoding="utf-8")
    ...
    if aligned_n_raw.empty or aligned_k_raw.empty:
        raise RuntimeError("No overlapping scored rows were found between Model N and Model K.")
    ...
    comparative_notes = [
        f"Model N source directory: {model_n_eval_dir}",
    ...
    (output_dir / "model_n_vs_model_k_dashboard.html").write_text(model_n_html(comparative_html), encoding="utf-8")

    print(f"Internal dashboard: {output_dir / 'model_n_internal_dashboard.html'}")
    print(f"Comparative dashboard: {output_dir / 'model_n_vs_model_k_dashboard.html'}")
