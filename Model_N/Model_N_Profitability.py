...
OUTPUT_FOLDER_NAME = "Model_N_Profitability_Outputs"
STRATEGIES = (
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Analyze Model N profitability versus Model K by taking favorable YES or NO trades. "
            "BUY_YES is allowed when p_model_n > p_model_k; BUY_NO is allowed when "
            "1 - p_model_n > 1 - p_model_k."
        )
    )
    parser.add_argument(
        "--model-n-raw-values",
...
def build_overlap(model_n: pd.DataFrame, model_k: pd.DataFrame) -> pd.DataFrame:
    ...
    overlap["yes_edge"] = overlap["p_model_n"] - overlap["p_model_k"]
    overlap["no_edge"] = (1.0 - overlap["p_model_n"]) - (1.0 - overlap["p_model_k"])
...
def build_trades(overlap: pd.DataFrame, *, edge_threshold: float) -> pd.DataFrame:
    return build_strategy_trades(
        overlap,
        strategy_id="model_n",
        strategy_label="Model N Hybrid",
        probability_col="p_model_n",
...
def summarize_segment(frame: pd.DataFrame, *, segment: str, overlap_rows: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "average_model_n_selected_side_probability": float(frame["model_probability_selected_side"].mean())
        if signal_rows
        else np.nan,
...
def write_readme(output_dir: Path, *, edge_threshold: float) -> None:
    text = f"""Model N Profitability Analysis

Signal rules:
BUY_YES when p_model_n - p_model_k > {edge_threshold}.
BUY_NO when (1 - p_model_n) - (1 - p_model_k) > {edge_threshold}.
...
<head>
  ...
  <title>Model N Strategy PnL Dashboard</title>
  {dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    ...
    <h1>Model N, Model A, and Model B PnL</h1>
    <p class="lead">
  ...
  <section class="panel">
    <p>
      Coverage is measured against the Model N / Model K overlapping scored-row universe. A row is one
...
def main() -> None:
    ...
    overlap.to_csv(output_dir / "model_n_model_k_overlap.csv", index=False)
