...
def load_profitability_module() -> Any:
    module_path = Path(__file__).resolve().parent / "Model_N_Profitability.py"
    spec = importlib.util.spec_from_file_location("model_n_profitability_base", module_path)
...
OUTPUT_FOLDER_NAME = "Model_N_Fee_Aware_Profitability_Outputs"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build fee-aware Model N profitability outputs across linear Model N, "
            "exponential Model N lambda=2, Model A, and Model B."
        )
    )
    parser.add_argument(
        "--model-n-raw-values",
...
<head>
  ...
  <title>Model N {html.escape(fee_model_label)} PnL - {html.escape(scenario_label)}</title>
  {base.dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    <p class="eyebrow">{html.escape(fee_model_label)} Model N Profitability</p>
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Model N {html.escape(fee_model_label)} Profitability Overview</title>
  {base.dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    ...
    <p class="lead">
      ...
      0.01 / 0.02 required edge across linear Model N, exponential Model N lambda=2, Model A, and Model B.
    </p>
...
def write_readme(output_dir: Path, *, fee_rate: float, round_up_to_cent: bool, fee_model_label: str) -> None:
    text = f"""Model N {fee_model_label} Profitability
...
def main() -> None:
    ...
    model_n = base.load_raw_values(args.model_n_raw_values.resolve(), model_name="Model N")
    model_k = base.load_raw_values(args.model_k_raw_values.resolve(), model_name="Model K")
    overlap = base.build_overlap(model_n, model_k)
