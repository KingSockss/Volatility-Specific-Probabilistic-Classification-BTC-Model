...
USER_AGENT = "model-e-high-vol-garch-studentt-builder/1.0"

OUTPUT_FOLDER_NAME = "Model_E_Output_Raw_Data"
EPS = 1e-12
MIN_VARIANCE = 1e-12
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build Model E raw hourly forecast files by combining settlement-defined Kalshi "
    ...
    parser.add_argument(
        ...
        type=int,
        default=240,
        help="Minimum number of regime-filtered 1-minute returns required before a Model E fit is allowed.",
    )
    parser.add_argument(
        "--isotonic-mc-trials",
        type=int,
...
def fit_isotonic_calibration_layer(
    ...
    *,
    ...
    if training_frame.empty:
        raise RuntimeError("No synthetic calibration examples were available for the Model E isotonic layer.")
    ...
    for schedule_idx, forecast in enumerate(schedule.to_dict(orient="records"), start=1):
        ...
        if need_refit:
            ...
            if regime_sample.n_returns < args.min_regime_observations:
                raise RuntimeError(
                    "Insufficient regime-filtered history for Model E fit at "
