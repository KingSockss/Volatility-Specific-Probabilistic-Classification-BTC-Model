...
OUTPUT_FOLDER_NAME = "Model_F_Isotonic_Diagnostics_Outputs"
EPS = 1e-12
...
def default_diagnostics_dir(root: Path) -> Path:
    return root / "Model_F" / OUTPUT_FOLDER_NAME


def default_volatility_segments_csv(root: Path) -> Optional[Path]:
    candidates = [
        root
        / "Model_F_Volatility_Decomposition_RT"
        / "Model_F_Volatility_Decomposition_RT_outputs"
        / "hourly_market_volatility_segments.csv",
        root
        / "Model_F_Volatility_Results_Dashboard"
        / "Model_F_Volatility_Dashboards_outputs"
        / "hourly_market_volatility_segments.csv",
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Model F isotonic calibration diagnostics from raw-vs-calibrated outputs."
    )
    parser.add_argument(
        "--model-f-eval-dir",
        type=Path,
        default=default_eval_dir(REPO_ROOT),
        help="Directory containing Model_F_Eval.py outputs, especially raw_values.csv.",
    )
    parser.add_argument(
        "--isotonic-diagnostics-dir",
        type=Path,
        default=default_diagnostics_dir(REPO_ROOT),
        help="Directory containing Model_F.py isotonic diagnostic CSVs.",
    )
    parser.add_argument(
    ...
    *,
    ...
    hve_sections = [
        section(
            ...
            "When Model_F.py has been rerun with the new instrumentation, the training comparison also filters "
...
<head>
  ...
  <title>Model F Isotonic Diagnostics</title>
  {dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    <h1>Model F Isotonic Diagnostics</h1>
    <p>
      This dashboard compares pre-isotonic raw probabilities against calibrated Model F probabilities,
...
def main() -> None:
    args = parse_args()
    eval_dir = args.model_f_eval_dir.resolve()
    ...
    if not raw_path.exists():
        raise FileNotFoundError(f"Model F eval raw_values.csv not found: {raw_path}")

    raw = pd.read_csv(raw_path)
    required_raw_cols = {"p_model_raw", "p_model_calibrated", "outcome"}
    if not required_raw_cols.issubset(raw.columns):
        warnings.append(
            "raw_values.csv does not contain p_model_raw/p_model_calibrated yet. "
            "Rerun Model_F.py and Model_F_Eval.py before using the backtest raw-vs-calibrated diagnostics."
        )
        backtest = pd.DataFrame()
    else:
        backtest = attach_volatility_context(
            add_backtest_error_columns(raw),
            args.volatility_segments_csv.resolve() if args.volatility_segments_csv else None,
        )
        if backtest.empty:
            warnings.append(
                "raw_values.csv has raw-vs-calibrated columns, but no non-null raw/calibrated rows. "
                "This usually means the copied seed outputs have not been regenerated with Model_F.py yet."
            )

    training_path = diagnostics_dir / "isotonic_training_examples.csv"
    training = load_training_examples(training_path)
    if training.empty:
        warnings.append(
            "isotonic_training_examples.csv is missing or empty. Rerun Model_F.py to save isotonic training diagnostics."
        )
    else:
        training = add_training_error_columns(training)

    curve_path = diagnostics_dir / "isotonic_curve_thresholds.csv"
    curve = pd.read_csv(curve_path) if curve_path.exists() else pd.DataFrame()
    if curve.empty:
        warnings.append("isotonic_curve_thresholds.csv is missing or empty. Latest isotonic curve chart is unavailable.")
    ...
    if not backtest.empty and hve_backtest.empty:
        warnings.append(
            ...
            "Run the Model_F volatility decomposition/dashboard first, or provide --volatility-segments-csv."
        )
    if not training.empty and hve_training.empty:
        warnings.append(
            "No high-volatility-extreme isotonic training examples were available. "
            "Rerun Model_F.py with the latest instrumentation, or check whether the selected training regime contains HVE minutes."
        )
    ...
    print(f"Model F isotonic diagnostics written to: {output_dir}")
