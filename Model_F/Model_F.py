...
USER_AGENT = "model-f-high-vol-garch-studentt-diagnostics-builder/1.0"

OUTPUT_FOLDER_NAME = "Model_F_Output_Raw_Data"
ISOTONIC_DIAGNOSTICS_FOLDER_NAME = "Model_F_Isotonic_Diagnostics_Outputs"
EPS = 1e-12
MIN_VARIANCE = 1e-12
DEFAULT_ISOTONIC_MC_TRIALS = 1_000
DEFAULT_ISOTONIC_MAX_TRAINING_MINUTES = 1_500
DEFAULT_SYNTHETIC_STRIKE_STEP = 250.0
DEFAULT_SYNTHETIC_STRIKE_OFFSET = -0.01
...
@dataclass
class IsotonicCalibrationLayer:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build Model F raw hourly forecast files by combining settlement-defined Kalshi "
            "contracts with a Student-t GARCH(1,1) Monte Carlo BTC model whose parameters are "
            "fit on trailing regime-filtered historical returns and then calibrated with an "
            "isotonic regression layer trained outside the Kalshi backtest range."
        )
    )
    parser.add_argument(
        "--settlement-csv",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(__file__).resolve().parent / ISOTONIC_DIAGNOSTICS_FOLDER_NAME,
        help="Directory where raw-vs-calibrated isotonic diagnostic CSVs will be written.",
    )
    parser.add_argument(
        "--symbol",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=240,
        help="Minimum number of regime-filtered 1-minute returns required before a Model F fit is allowed.",
    )
    parser.add_argument(
        "--isotonic-mc-trials",
        type=int,
        default=DEFAULT_ISOTONIC_MC_TRIALS,
        help="Monte Carlo terminal paths per synthetic calibration example used by the isotonic layer.",
    )
    parser.add_argument(
        "--isotonic-max-training-minutes",
        type=int,
        default=DEFAULT_ISOTONIC_MAX_TRAINING_MINUTES,
        help=(
            "Maximum number of regime-filtered historical minutes sampled for each isotonic refit. "
            "Each minute contributes ATM, OTM+1, and OTM-1 synthetic contract examples."
        ),
    )
    parser.add_argument(
        "--min-isotonic-observations",
        type=int,
        default=240,
        help="Minimum number of synthetic contract outcomes required before an isotonic layer is accepted.",
    )
    parser.add_argument(
        "--synthetic-strike-step",
        type=float,
        default=DEFAULT_SYNTHETIC_STRIKE_STEP,
        help="Strike spacing used to build synthetic ATM/OTM calibration contracts from historical BTC prices.",
    )
    parser.add_argument(
        "--synthetic-strike-offset",
        type=float,
        default=DEFAULT_SYNTHETIC_STRIKE_OFFSET,
        help="Strike offset used with --synthetic-strike-step; -0.01 matches the Kalshi BTC strike ladder style.",
    )
...
def select_regime_training_sample(
    ...
    *,
    ...
    regime_lookback_days: int,
    segment_key: str,
    training_cutoff_utc: Optional[pd.Timestamp] = None,
    ...
    lookback_end = current_hour_start
    if training_cutoff_utc is not None:
        lookback_end = min(lookback_end, training_cutoff_utc)
    ...
    sample_columns = [column for column in sample_columns if column in prices.columns]

    eligible = prices[
        (prices["hour_start_utc"] >= lookback_start)
        & (prices["hour_start_utc"] < lookback_end)
...
def calibrated_probability(
    ...
    calibration_layer: IsotonicCalibrationLayer,
) -> Optional[float]:
    if raw_probability is None or pd.isna(raw_probability):
        return None
    value = float(raw_probability)
    calibrated = float(np.interp(value, calibration_layer.x_thresholds, calibration_layer.y_thresholds))
    return min(max(calibrated, 0.0), 1.0)


def calibrated_probability_from_terminals(
    terminals: np.ndarray,
    strike: Optional[float],
    calibration_layer: IsotonicCalibrationLayer,
) -> Optional[float]:
    return calibrated_probability(probability_from_terminals(terminals, strike), calibration_layer)


def raw_and_calibrated_probability_from_terminals(
    terminals: np.ndarray,
    strike: Optional[float],
    calibration_layer: IsotonicCalibrationLayer,
) -> Tuple[Optional[float], Optional[float]]:
    raw_probability = probability_from_terminals(terminals, strike)
    return raw_probability, calibrated_probability(raw_probability, calibration_layer)


def synthetic_contract_strikes(reference_price: float, strike_step: float, strike_offset: float) -> Tuple[float, float, float]:
    if strike_step <= 0.0:
        raise ValueError("--synthetic-strike-step must be positive.")
    atm = round((reference_price - strike_offset) / strike_step) * strike_step + strike_offset
    return float(atm), float(atm + strike_step), float(atm - strike_step)


def next_variance_by_training_price_index(
    sample: RegimeTrainingSample,
    params: GarchStudentTParams,
) -> Dict[int, float]:
    variances: Dict[int, float] = {}
    rows = sample.rows.sort_values("price_index").reset_index(drop=True)
    if rows.empty:
        return variances

    block_ids = rows["price_index"].diff().fillna(1).ne(1).cumsum()
    for _, block in rows.groupby(block_ids, sort=False):
        returns = block["log_return"].to_numpy(dtype=float)
        if len(returns) == 0:
            continue

        eps = returns - params.mu
        current_variance = initial_variance_for_fit(returns, params)
        for idx, price_index in enumerate(block["price_index"].to_numpy(dtype=int)):
            if idx > 0:
                current_variance = max(
                    params.omega + params.alpha * (eps[idx - 1] ** 2) + params.beta * current_variance,
                    MIN_VARIANCE,
                )
            variances[int(price_index)] = max(
                params.omega + params.alpha * (eps[idx] ** 2) + params.beta * current_variance,
                MIN_VARIANCE,
            )
    return variances


def evenly_sample_rows(frame: pd.DataFrame, max_rows: int) -> pd.DataFrame:
...
def build_isotonic_training_frame(
    prices: pd.DataFrame,
    sample: RegimeTrainingSample,
    params: GarchStudentTParams,
    rng: np.random.Generator,
    *,
    ...
    rows = sample.rows.sort_values("price_index").reset_index(drop=True)
    if rows.empty:
        return pd.DataFrame(columns=["raw_probability", "outcome", "minute_price_index", "contract_label"])

    spots = prices["spot_open"].to_numpy(dtype=float)
    datetimes = prices["datetime_utc"].to_numpy()
    hour_start_prices = (
        prices.drop_duplicates(subset=["hour_start_utc"], keep="first")
        .set_index("hour_start_utc")["spot_open"]
        .to_dict()
    )
    next_variances = next_variance_by_training_price_index(sample, params)

    candidate_rows: List[Dict[str, Any]] = []
    for row in rows.to_dict(orient="records"):
        forecast_dt = row["datetime_utc"]
        price_index = int(row["price_index"])
        horizon_steps = 60 - int(forecast_dt.minute)
        terminal_index = price_index + horizon_steps
        if horizon_steps <= 0 or terminal_index >= len(spots):
            continue
        terminal_dt = pd.Timestamp(datetimes[terminal_index])
        if terminal_dt.tzinfo is None:
            terminal_dt = terminal_dt.tz_localize(UTC)
        if terminal_dt >= training_cutoff_utc:
            continue
        if price_index not in next_variances:
            continue
        reference_price = hour_start_prices.get(row["hour_start_utc"], row["spot_open"])
        candidate_rows.append(
            {
                "price_index": price_index,
                "forecast_dt": forecast_dt,
                "horizon_steps": horizon_steps,
                "current_spot": float(row["spot_open"]),
                "terminal_spot": float(spots[terminal_index]),
                "reference_price": float(reference_price),
                "next_variance": float(next_variances[price_index]),
                "training_primary_volatility_band": row.get("primary_volatility_band"),
                "training_rolling_percentile_rank": row.get("rolling_percentile_rank"),
                "training_is_low_volatility": row.get("is_low_volatility"),
                "training_is_standard_volatility": row.get("is_standard_volatility"),
                "training_is_high_volatility": row.get("is_high_volatility"),
                "training_is_low_volatility_extreme": row.get("is_low_volatility_extreme"),
                "training_is_high_volatility_extreme": row.get("is_high_volatility_extreme"),
            }
        )

    candidate_frame = evenly_sample_rows(pd.DataFrame(candidate_rows), max_training_minutes)
    ...
    for row in candidate_frame.to_dict(orient="records"):
        terminals = simulate_terminal_prices(
            ...
            initial_variance=float(row["next_variance"]),
            horizon_steps=int(row["horizon_steps"]),
            params=params,
            n_trials=mc_trials,
            rng=rng,
        )
        strikes = synthetic_contract_strikes(
            reference_price=float(row["reference_price"]),
            strike_step=strike_step,
            strike_offset=strike_offset,
        )
        for label, strike in zip(("ATM", "OTM+1", "OTM-1"), strikes):
            raw_probability = probability_from_terminals(terminals, strike)
            if raw_probability is None:
                continue
            calibration_rows.append(
                {
                    "training_forecast_datetime_utc": row["forecast_dt"],
                    "training_hour_start_utc": row["forecast_dt"].floor("h"),
                    "terminal_datetime_utc": pd.Timestamp(datetimes[int(row["price_index"]) + int(row["horizon_steps"])]),
                    "horizon_steps": int(row["horizon_steps"]),
                    "current_spot": float(row["current_spot"]),
                    "terminal_spot": float(row["terminal_spot"]),
                    "reference_price": float(row["reference_price"]),
                    "synthetic_strike": float(strike),
                    "next_variance": float(row["next_variance"]),
                    "training_primary_volatility_band": row.get("training_primary_volatility_band"),
                    "training_rolling_percentile_rank": row.get("training_rolling_percentile_rank"),
                    "training_is_low_volatility": row.get("training_is_low_volatility"),
                    "training_is_standard_volatility": row.get("training_is_standard_volatility"),
                    "training_is_high_volatility": row.get("training_is_high_volatility"),
                    "training_is_low_volatility_extreme": row.get("training_is_low_volatility_extreme"),
                    "training_is_high_volatility_extreme": row.get("training_is_high_volatility_extreme"),
                    "raw_probability": float(raw_probability),
                    "outcome": int(float(row["terminal_spot"]) >= strike),
                    "minute_price_index": int(row["price_index"]),
                    "contract_label": label,
                    "synthetic_moneyness": float(strike / row["current_spot"] - 1.0),
                    "terminal_return": float(math.log(row["terminal_spot"] / row["current_spot"])),
                }
            )
...
def fit_isotonic_calibration_layer(
    training_frame: pd.DataFrame,
    *,
    min_observations: int,
) -> IsotonicCalibrationLayer:
    if training_frame.empty:
        raise RuntimeError("No synthetic calibration examples were available for the Model F isotonic layer.")

    work = training_frame[["raw_probability", "outcome", "minute_price_index"]].copy()
    work["raw_probability"] = pd.to_numeric(work["raw_probability"], errors="coerce")
    work["outcome"] = pd.to_numeric(work["outcome"], errors="coerce")
    work = work.dropna(subset=["raw_probability", "outcome"])
    work = work[(work["raw_probability"] >= 0.0) & (work["raw_probability"] <= 1.0)]
    work = work[work["outcome"].isin([0, 1])]
    if len(work) < min_observations:
        raise RuntimeError(
            f"Only {len(work)} isotonic calibration examples were available; "
            f"need at least {min_observations}."
        )

    x = work["raw_probability"].to_numpy(dtype=float)
    ...
    sums = np.bincount(inverse, weights=y_sorted).astype(float)
    means = sums / np.maximum(weights, 1.0)

    block_x: List[float] = []
    block_y: List[float] = []
    block_weight: List[float] = []
    for value_x, value_y, value_weight in zip(unique_x, means, weights):
        block_x.append(float(value_x))
        block_y.append(float(value_y))
        block_weight.append(float(value_weight))
    ...
    order = np.argsort(x_thresholds, kind="mergesort")
    x_thresholds = x_thresholds[order]
    y_thresholds = y_thresholds[order]
    if len(x_thresholds) == 1:
        x_thresholds = np.array([0.0, 1.0], dtype=float)
        y_thresholds = np.repeat(y_thresholds[0], 2)

    return IsotonicCalibrationLayer(
        x_thresholds=x_thresholds,
        y_thresholds=y_thresholds,
        n_samples=int(len(work)),
        n_minutes=int(work["minute_price_index"].nunique()),
    )


def reset_isotonic_diagnostic_outputs(output_dir: Path) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    for filename in [
        "isotonic_training_examples.csv",
        "isotonic_curve_thresholds.csv",
        "isotonic_refit_summary.csv",
    ]:
...
def append_diagnostic_csv(frame: pd.DataFrame, path: Path) -> None:
    if frame.empty:
        return
...
def enrich_isotonic_training_diagnostics(
    training_frame: pd.DataFrame,
    *,
    calibration_layer: IsotonicCalibrationLayer,
    refit_id: int,
    schedule_idx: int,
    event_ticker: str,
    forecast_datetime_utc: pd.Timestamp,
) -> pd.DataFrame:
    out = training_frame.copy()
    if out.empty:
        return out
    out.insert(0, "refit_id", refit_id)
    out.insert(1, "schedule_idx", schedule_idx)
    out.insert(2, "refit_event_ticker", event_ticker)
    out.insert(3, "refit_forecast_datetime_utc", forecast_datetime_utc)
    out["calibrated_probability"] = out["raw_probability"].map(
        lambda value: calibrated_probability(float(value), calibration_layer)
    )
    out["calibration_shift"] = out["calibrated_probability"] - out["raw_probability"]
    out["squared_error_raw"] = (out["raw_probability"] - out["outcome"]) ** 2
    out["squared_error_calibrated"] = (out["calibrated_probability"] - out["outcome"]) ** 2
    out["absolute_error_raw"] = (out["raw_probability"] - out["outcome"]).abs()
    out["absolute_error_calibrated"] = (out["calibrated_probability"] - out["outcome"]).abs()
    out["log_loss_raw"] = -(
        out["outcome"] * np.log(out["raw_probability"].clip(EPS, 1 - EPS))
        + (1 - out["outcome"]) * np.log((1 - out["raw_probability"]).clip(EPS, 1 - EPS))
    )
    out["log_loss_calibrated"] = -(
        out["outcome"] * np.log(out["calibrated_probability"].clip(EPS, 1 - EPS))
        + (1 - out["outcome"]) * np.log((1 - out["calibrated_probability"]).clip(EPS, 1 - EPS))
    )
    return out


def isotonic_curve_threshold_frame(
    *,
    calibration_layer: IsotonicCalibrationLayer,
    refit_id: int,
    schedule_idx: int,
    ...
    return pd.DataFrame(
        {
            ...
            "refit_event_ticker": event_ticker,
            "refit_forecast_datetime_utc": forecast_datetime_utc,
            "threshold_index": np.arange(len(calibration_layer.x_thresholds), dtype=int),
            "raw_probability_threshold": calibration_layer.x_thresholds,
            "calibrated_probability_threshold": calibration_layer.y_thresholds,
            "calibration_shift": calibration_layer.y_thresholds - calibration_layer.x_thresholds,
        }
    )


def isotonic_refit_summary_frame(
    *,
    refit_id: int,
    schedule_idx: int,
    ...
    event_ticker: str,
    forecast_datetime_utc: pd.Timestamp,
    training_segment_label: str,
    regime_sample: RegimeTrainingSample,
    state_returns: np.ndarray,
    fit_state: GarchFitState,
    calibration_layer: IsotonicCalibrationLayer,
        ...
        [
            {
                ...
                "forecast_datetime_utc": forecast_datetime_utc,
                "training_segment": training_segment_label,
                "lookback_start_utc": regime_sample.lookback_start_utc,
                "lookback_end_utc": regime_sample.lookback_end_utc,
                "garch_training_returns": regime_sample.n_returns,
                "garch_training_blocks": regime_sample.n_blocks,
                "garch_training_hours": regime_sample.n_hours,
                "state_history_returns": int(len(state_returns)),
                "neg_log_likelihood": fit_state.neg_log_likelihood,
                "mu": fit_state.params.mu,
                "omega": fit_state.params.omega,
                "alpha": fit_state.params.alpha,
                "beta": fit_state.params.beta,
                "nu": fit_state.params.nu,
                "isotonic_examples": calibration_layer.n_samples,
                "isotonic_minutes": calibration_layer.n_minutes,
                "isotonic_curve_points": int(len(calibration_layer.x_thresholds)),
                "training_raw_probability_mean": float(diagnostics["raw_probability"].mean()) if not diagnostics.empty else np.nan,
                "training_calibrated_probability_mean": float(diagnostics["calibrated_probability"].mean()) if not diagnostics.empty else np.nan,
                "training_observed_frequency": float(diagnostics["outcome"].mean()) if not diagnostics.empty else np.nan,
                "training_brier_raw": float(diagnostics["squared_error_raw"].mean()) if not diagnostics.empty else np.nan,
                "training_brier_calibrated": float(diagnostics["squared_error_calibrated"].mean()) if not diagnostics.empty else np.nan,
                "training_log_loss_raw": float(diagnostics["log_loss_raw"].mean()) if not diagnostics.empty else np.nan,
                "training_log_loss_calibrated": float(diagnostics["log_loss_calibrated"].mean()) if not diagnostics.empty else np.nan,
            }
        ]
    )
...
def build_output_frame(rows: Iterable[Dict[str, Any]]) -> pd.DataFrame:
    ...
    ordered_columns = [
        ...
        "ATM_strike",
        "ATM_raw_price",
        "ATM_calibrated_price",
        "ATM_price",
        "OTM+1_market_ticker",
        "OTM+1_strike",
        "OTM+1_raw_price",
        "OTM+1_calibrated_price",
        "OTM+1_price",
        "OTM-1_market_ticker",
        "OTM-1_strike",
        "OTM-1_raw_price",
...
def main() -> None:
    ...
    if args.isotonic_mc_trials <= 1:
        raise ValueError("--isotonic-mc-trials must be greater than 1.")
    if args.isotonic_max_training_minutes <= 0:
        raise ValueError("--isotonic-max-training-minutes must be positive.")
    if args.min_isotonic_observations <= 0:
        raise ValueError("--min-isotonic-observations must be positive.")
    if args.synthetic_strike_step <= 0.0:
        raise ValueError("--synthetic-strike-step must be positive.")

    diagnostics_dir = args.isotonic_diagnostics_dir.resolve()
    reset_isotonic_diagnostic_outputs(diagnostics_dir)
    ...
    backtest_start_utc = settlements["forecast_hour_start_datetime_utc"].min()
    backtest_end_utc = settlements["event_datetime_utc"].max()
    ...
    print(
        ...
        f"training_cutoff={backtest_start_utc.isoformat()} "
        f"backtest_end={backtest_end_utc.isoformat()}"
    )

    rng = np.random.default_rng(args.seed)
    isotonic_rng = np.random.default_rng(args.seed + 1_000_003)
    fit_state: Optional[GarchFitState] = None
    calibration_layer: Optional[IsotonicCalibrationLayer] = None
    ...
    for schedule_idx, forecast in enumerate(schedule.to_dict(orient="records"), start=1):
        ...
        if need_refit:
            ...
            regime_sample = select_regime_training_sample(
                ...
                training_cutoff_utc=backtest_start_utc,
            )
            if regime_sample.n_returns < args.min_regime_observations:
                raise RuntimeError(
                    "Insufficient regime-filtered history for Model F fit at "
            ...
            isotonic_training = build_isotonic_training_frame(
                prices=prices,
                sample=regime_sample,
                params=fit_state.params,
                rng=isotonic_rng,
                training_cutoff_utc=backtest_start_utc,
                max_training_minutes=args.isotonic_max_training_minutes,
                mc_trials=args.isotonic_mc_trials,
                strike_step=args.synthetic_strike_step,
                strike_offset=args.synthetic_strike_offset,
            )
            calibration_layer = fit_isotonic_calibration_layer(
                isotonic_training,
                min_observations=args.min_isotonic_observations,
            )
            refit_id += 1
            isotonic_training_diagnostics = enrich_isotonic_training_diagnostics(
                isotonic_training,
                calibration_layer=calibration_layer,
                refit_id=refit_id,
                schedule_idx=schedule_idx,
                event_ticker=str(forecast["Event"]),
                forecast_datetime_utc=forecast_dt,
            )
            append_diagnostic_csv(
                isotonic_training_diagnostics,
                diagnostics_dir / "isotonic_training_examples.csv",
            )
            append_diagnostic_csv(
                isotonic_curve_threshold_frame(
                    calibration_layer=calibration_layer,
                    refit_id=refit_id,
                    schedule_idx=schedule_idx,
                    event_ticker=str(forecast["Event"]),
            ...
            append_diagnostic_csv(
                isotonic_refit_summary_frame(
                    refit_id=refit_id,
                    schedule_idx=schedule_idx,
                    total_schedule_rows=total,
                    event_ticker=str(forecast["Event"]),
                    forecast_datetime_utc=forecast_dt,
                    training_segment_label=training_segment["label"],
                    regime_sample=regime_sample,
                    state_returns=state_returns,
                    fit_state=fit_state,
                    calibration_layer=calibration_layer,
                    diagnostics=isotonic_training_diagnostics,
                ),
                diagnostics_dir / "isotonic_refit_summary.csv",
            )
            ...
            print(
                ...
                f"| isotonic_examples={calibration_layer.n_samples} "
                f"isotonic_minutes={calibration_layer.n_minutes}"
            )
        else:
        ...
        atm_raw, atm_calibrated = raw_and_calibrated_probability_from_terminals(
            terminals, forecast["ATM_strike"], calibration_layer
        )
        otm_plus_raw, otm_plus_calibrated = raw_and_calibrated_probability_from_terminals(
            terminals, forecast["OTM+1_strike"], calibration_layer
        )
        otm_minus_raw, otm_minus_calibrated = raw_and_calibrated_probability_from_terminals(
            terminals, forecast["OTM-1_strike"], calibration_layer
        )
        ...
        fitted_forecasts.append(
            {
                ...
                "ATM_raw_price": atm_raw,
                "ATM_calibrated_price": atm_calibrated,
                "ATM_price": atm_calibrated,
                "OTM+1_market_ticker": forecast["OTM+1_market_ticker"],
                "OTM+1_strike": forecast["OTM+1_strike"],
                "OTM+1_raw_price": otm_plus_raw,
                "OTM+1_calibrated_price": otm_plus_calibrated,
                "OTM+1_price": otm_plus_calibrated,
                "OTM-1_market_ticker": forecast["OTM-1_market_ticker"],
                "OTM-1_strike": forecast["OTM-1_strike"],
                "OTM-1_raw_price": otm_minus_raw,
                "OTM-1_calibrated_price": otm_minus_calibrated,
                "OTM-1_price": otm_minus_calibrated,
            }
        )
    ...
    print(f"Isotonic diagnostics directory: {diagnostics_dir}")
