...
EPS = 1e-15
ASSUMPTIONS = (
    "Model F forecast values are interpreted as YES probabilities. CSV values greater than 1 and no greater than 100 "
    ...
    "Model F probabilities include an isotonic regression layer fit only on high-volatility historical minutes "
    "outside the Kalshi settlement/pricing backtest range.",
    "Model F forecast CSVs preserve pre-isotonic raw probabilities in *_raw_price columns and final "
    "calibrated probabilities in *_calibrated_price and *_price columns.",
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate Model F BTC hourly contract probabilities against official "
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "model_F_Evals_Outputs",
        help="Directory for Model F evaluation output files.",
    )
    parser.add_argument(
        "--skip-binance-audit",
        action="store_true",
...
def load_kalshi_price_outputs(price_dir: Path) -> pd.DataFrame:
    ...
    for label in CONTRACT_LABELS:
        ...
        if raw_price_col in wide.columns:
            source_cols.append(raw_price_col)
        if calibrated_price_col in wide.columns:
            source_cols.append(calibrated_price_col)
        ...
        if raw_price_col in part.columns:
            rename_cols[raw_price_col] = "p_model_raw"
        if calibrated_price_col in part.columns:
            rename_cols[calibrated_price_col] = "p_model_calibrated"
        ...
        if "p_model_raw" not in part.columns:
            part["p_model_raw"] = np.nan
        if "p_model_calibrated" not in part.columns:
            part["p_model_calibrated"] = np.nan
        part["contract_label"] = label
        part["p_kalshi"], scale = normalize_kalshi_probability(part["p_kalshi_raw"])
        part["p_model_raw"] = pd.to_numeric(part["p_model_raw"], errors="coerce").clip(0.0, 1.0)
        part["p_model_calibrated"] = pd.to_numeric(part["p_model_calibrated"], errors="coerce").clip(0.0, 1.0)
...
def write_individual_metric_files(
    *,
    ...
    (output_dir / "model_f_assumptions.txt").write_text("\n".join(ASSUMPTIONS) + "\n", encoding="utf-8")
...
def svg_line_chart(
    ...
    *,
    ...
    <svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(title)}">
      ...
      {circles}
      <text x="{plot_left}" y="{height - 12}" class="axis-label">Mean Model F probability</text>
...
def svg_histogram(
    ...
    *,
    ...
    <svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(title)}">
      ...
      <line x1="{plot_left}" y1="{plot_top}" x2="{plot_left}" y2="{plot_bottom}" class="axis" />
      {''.join(bars)}
      <text x="{plot_left}" y="{height - 12}" class="axis-label">Model F probability</text>
...
def plotly_report_charts(raw: pd.DataFrame, calibration: pd.DataFrame) -> str:
    ...
    fig = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=("Calibration", "Model F Probability Distribution"),
    ...
    fig.update_xaxes(range=[0, 1], title_text="Mean Model F probability", row=1, col=1)
    fig.update_yaxes(range=[0, 1], title_text="Observed frequency", row=1, col=1)
    fig.update_xaxes(range=[0, 1], title_text="Model F probability", row=1, col=2)
...
def fallback_report_charts(raw: pd.DataFrame, calibration: pd.DataFrame) -> str:
    ...
    return (
        ...
        + svg_histogram(raw, value_col="p_kalshi", title="Model F Probability Distribution")
        + "</div>"
    )
...
<main>
  ...
  <p>
    Forecasts are minute-level Model F YES probabilities from the hourly forecast files. Reality is the
...
def main() -> None:
    ...
    summary_path = output_dir / "model_f_summary.html"
    ...
    summary_with_coverage_path = output_dir / "model_f_summary_with_coverage.html"
    ...
    print(f"Model F outputs saved to: {output_dir}")
