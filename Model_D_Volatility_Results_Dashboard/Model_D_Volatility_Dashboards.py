...
from Model_D.Model_D_Eval import (
    ASSUMPTIONS as MODEL_D_ASSUMPTIONS,
    HAS_PLOTLY,
    attach_outcomes as attach_outcomes_d,
    ...
    go,
    load_kalshi_price_outputs as load_model_d_price_outputs,
    load_settlements,
    make_subplots,
)
...
OUTPUT_FOLDER_NAME = "Model_D_Volatility_Dashboards_outputs"
MODEL_D_TOTAL_LABEL = "Model D Total"
MODEL_D_RT_LABEL = "Model D RT"
MODEL_K_RT_LABEL = "Model K RT"

COLOR_MODEL_D = "#6cb6ff"
COLOR_MODEL_D_RT = "#ff8f70"
...
def model_d_rt_output_root(root: Path) -> Path:
    return root / "Model_D_Volatility_Decomposition_RT" / "Model_D_Volatility_Decomposition_RT_outputs"
...
def default_hourly_states_csv(root: Path) -> Optional[Path]:
    return first_existing(
        [
            model_d_rt_output_root(root) / "hourly_market_volatility_segments.csv",
...
def default_binance_cache_csv(root: Path) -> Optional[Path]:
    return first_existing(
        [
            model_d_rt_output_root(root) / "binance_1m_klines.csv",
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build dark-mode volatility comparison dashboards for Model D real-time volatility "
            "segments versus Model D overall and versus Model K real-time volatility segments."
        )
    )
    parser.add_argument(
        "--model-d-price-dir",
        type=Path,
        default=root / "Model_D" / "Model_D_Output_Raw_Data",
        help="Folder containing Model_D.py hourly raw forecast CSVs.",
    )
    parser.add_argument(
        "--model-k-price-dir",
        type=Path,
...
def dashboard_style() -> str:
    ...
    :root {{
      ...
      --ink: {COLOR_TEXT};
      --muted: {COLOR_MUTED};
      --model-d: {COLOR_MODEL_D};
      --model-d-rt: {COLOR_MODEL_D_RT};
    ...
    .swatch.total {{
      background: var(--model-d);
    }}
    .swatch.vol {{
      background: var(--model-d-rt);
    }}
    .swatch.k {{
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Model D Volatility Dashboards</title>
  {dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    <p class="eyebrow">Model D Dashboard Index</p>
    <h1>Model D Volatility Dashboard Families</h1>
    <p class="lead">
      This output contains two dark-mode dashboard families built on the same real-time volatility
      segmentation: <code>Model D RT vs Model D Total</code> and <code>Model D RT vs Model K RT</code>.
    </p>
    <div class="hero-pills">
      {pill_html(MODEL_D_TOTAL_LABEL, "total")}
      {pill_html(MODEL_D_RT_LABEL, "vol")}
  ...
  <section class="panel">
    <ul class="links">
      <li><a href="model_d_rt_vs_total/index.html">Model D RT vs Model D Total</a></li>
      <li><a href="model_d_rt_vs_model_k_rt/index.html">Model D RT vs Model K RT</a></li>
    </ul>
  </section>
</main>
</body>
...
def main() -> None:
    ...
    model_d_forecasts = load_model_d_price_outputs(resolve_path(root, args.model_d_price_dir))
    ...
    if preferred_states_path and preferred_states_path.exists():
        ...
        model_d_forecasts = model_d_forecasts[model_d_forecasts["event_ticker"].astype(str).isin(allowed_events)].copy()
    ...
    model_d_raw, model_d_unmatched = attach_outcomes_d(model_d_forecasts, outcomes)
    ...
    model_d_total_bundle = build_bundle(
        name=MODEL_D_TOTAL_LABEL,
        raw=model_d_raw,
        forecasts=model_d_forecasts,
        unmatched=model_d_unmatched,
    ...
    assumptions_a = list(MODEL_D_ASSUMPTIONS) + list(volatility_assumptions(thresholds))
    assumptions_cross = list(dict.fromkeys([*MODEL_D_ASSUMPTIONS, *MODEL_K_ASSUMPTIONS, *volatility_assumptions(thresholds)]))
    shared_notes = [
        *source_notes,
        *state_notes,
    ...
    family_a_root = output_root / "model_d_rt_vs_total"
    family_k_root = output_root / "model_d_rt_vs_model_k_rt"
    ...
    for segment in SEGMENTS:
        ...
        model_d_segment_forecasts = slice_by_event_ids(model_d_forecasts, event_ids)
        model_d_segment_raw = slice_by_event_ids(model_d_raw, event_ids)
        model_d_segment_unmatched = slice_by_event_ids(model_d_unmatched, event_ids)
        model_d_segment_bundle = build_bundle(
            name=f"{MODEL_D_RT_LABEL}: {segment['name']}",
            raw=model_d_segment_raw,
            forecasts=model_d_segment_forecasts,
            unmatched=model_d_segment_unmatched,
            calibration_bins=args.calibration_bins,
            classification_threshold=args.classification_threshold,
            skip_binance_audit=args.skip_binance_audit,
        ...
        segment_match_rate = model_d_segment_bundle["coverage"].loc[
            model_d_segment_bundle["coverage"]["scope"] == "overall", "match_rate"
        ].iloc[0]
        total_match_rate = model_d_total_bundle["coverage"].loc[
            model_d_total_bundle["coverage"]["scope"] == "overall", "match_rate"
        ].iloc[0]
        ...
        write_segment_artifacts(
            ...
            page_title=f"Model D RT vs Total: {segment['name']}",
            eyebrow="Model D Volatility Dashboard",
            heading=f"{segment['name']} vs Model D Total",
            lead_html=(
                "This page compares the <code>Model_D.py</code> full-sample evaluation against the "
                f"<code>{html.escape(segment['name'])}</code> slice from "
                "<code>Model_D_Volatility_Decomposition_RT.py</code>. The volatility slice is defined as: "
                f"{html.escape(segment['rule'])}"
            ),
            hero_pills=[
                pill_html(f"{segment['name']} hourly markets: {model_d_segment_bundle['n_hourly_markets']:,}", "vol"),
                pill_html(f"Model D total hourly markets: {total_hourly_markets:,}", "total"),
            ...
            focus_bundle=model_d_segment_bundle,
            benchmark_bundle=model_d_total_bundle,
            focus_label=f"{MODEL_D_RT_LABEL}: {segment['name']}",
            benchmark_label=MODEL_D_TOTAL_LABEL,
            focus_color=COLOR_MODEL_D_RT,
            benchmark_color=COLOR_MODEL_D,
            thresholds=thresholds,
            assumptions=assumptions_a,
            notes=[
                *shared_notes,
                "The Model D total baseline includes the volatility segment itself. This is a subset-vs-full-sample comparison, not a disjoint holdout.",
            ],
            topline_note=(
                "The Model D total baseline includes the volatility segment itself. "
                "This dashboard shows how the subset behaves relative to the full scored Model D sample."
            ),
        )
        ...
        overlap_keys = overlap_comparison_keys(model_d_segment_raw, model_k_segment_raw)
        model_d_overlap_bundle = build_bundle(
            name=f"{MODEL_D_RT_LABEL}: {segment['name']}",
            raw=slice_frame_to_comparison_keys(model_d_segment_raw, overlap_keys),
            forecasts=slice_frame_to_comparison_keys(model_d_segment_forecasts, overlap_keys),
            unmatched=slice_frame_to_comparison_keys(model_d_segment_unmatched, overlap_keys),
            calibration_bins=args.calibration_bins,
            classification_threshold=args.classification_threshold,
            skip_binance_audit=args.skip_binance_audit,
            n_hourly_markets=int(slice_frame_to_comparison_keys(model_d_segment_raw, overlap_keys)["event_ticker"].nunique()),
        )
        ...
        overlap_contracts = int(model_d_overlap_bundle["raw"]["event_contract_id"].nunique()) if not model_d_overlap_bundle["raw"].empty else 0
        match_rate_a_overlap = model_d_overlap_bundle["coverage"].loc[
            model_d_overlap_bundle["coverage"]["scope"] == "overall", "match_rate"
        ].iloc[0]
        ...
        write_segment_artifacts(
            ...
            page_title=f"Model D RT vs Model K RT: {segment['name']}",
            eyebrow="Cross-Model Volatility Dashboard",
            heading=f"Model D RT vs Model K RT: {segment['name']}",
            lead_html=(
                "This page compares <code>Model_D_Volatility_Decomposition_RT.py</code> against "
            ...
            focus_bundle=model_d_overlap_bundle,
            ...
            topline_note=(
                ...
                "so the Model D and Model K values are directly comparable inside the same volatility slice."
            ),
        )
    ...
    (family_a_root / "index.html").write_text(
        build_family_index_html(
            title="Model D RT vs Total Dashboards",
            eyebrow="Model D Dashboard Index",
            heading="Model D Total vs Volatility Segment Comparisons",
            lead_html=(
                "This output keeps the same scoring pipeline from <code>Model_D.py</code> and "
                "<code>Model_D_Volatility_Decomposition_RT.py</code>, but reorganizes the results into "
                "comparison dashboards that line up each volatility slice against the full Model D baseline."
            ),
            legend_pills=[
                pill_html(MODEL_D_TOTAL_LABEL, "total"),
                pill_html(MODEL_D_RT_LABEL, "vol"),
    ...
    (family_k_root / "index.html").write_text(
        build_family_index_html(
            title="Model D RT vs Model K RT Dashboards",
            eyebrow="Cross-Model Dashboard Index",
            heading="Model D RT vs Model K RT Comparisons",
            lead_html=(
                "This output compares <code>Model_D_Volatility_Decomposition_RT.py</code> against "
    ...
    print(f"Model D RT vs total index: {family_a_root / 'index.html'}")
    print(f"Model D RT vs Model K RT index: {family_k_root / 'index.html'}")
