...
from Model_F.Model_F_Eval import (  # noqa: E402
    ASSUMPTIONS,
    attach_outcomes,
...
def default_hourly_states_csv(root: Path) -> Path | None:
    return first_existing(
        [
            ...
            root / "Model_F_Volatility_Decomposition_RT" / "Model_F_Volatility_Decomposition_RT_outputs" / "hourly_market_volatility_segments.csv",
        ]
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run Model F outputs separately across hourly Kalshi BTC market states "
    ...
    parser.add_argument(
        ...
        help="Folder containing Model_F.py hourly raw forecast CSV outputs.",
    )
    parser.add_argument(
        "--settlement-csv",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "Model_F_Volatility_Decomposition_RT_outputs",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "If present and it covers the filtered Model F event universe, it is reused."
        ),
    )
    parser.add_argument(
...
def retitle_summary_html(document: str, *, heading: str, output_folder_name: str) -> str:
    updated = document.replace("<title>Model F Summary</title>", f"<title>{html.escape(heading)}</title>")
    updated = updated.replace(
        "<h1>Model F: Probability Evaluation</h1>",
        f"<h1>{html.escape(heading)}</h1>",
    )
    updated = updated.replace(
        "Individual CSV outputs are saved next to this report in <code>model_F_Evals_Outputs</code>:",
...
def retitle_time_bucket_html(document: str, *, heading: str) -> str:
    updated = document.replace(
        "<title>Model F Market Minute Bucket Summary</title>",
        f"<title>{html.escape(heading)}</title>",
    )
    updated = updated.replace(
        "<h1>Model F: Market Minute Buckets</h1>",
        f"<h1>{html.escape(heading)}</h1>",
    )
    return apply_dark_mode_html(updated)
...
def build_segment_outputs(
    *,
    ...
    (segment_dir / "model_f_assumptions.txt").write_text("\n".join(all_assumptions) + "\n", encoding="utf-8")
    ...
    summary_heading = f"Model F RT: {segment_name}"
    ...
    (segment_dir / "model_f_summary.html").write_text(
    ...
    (segment_dir / "model_f_summary_with_coverage.html").write_text(
        retitle_summary_html(
            summary_with_coverage_html,
            heading=f"Model F RT: {segment_name} With Coverage",
    ...
    (segment_dir / "time_bucket_summary.html").write_text(
        retitle_time_bucket_html(
            time_bucket_html,
            heading=f"Model F RT: {segment_name} Market Minute Buckets",
        ),
        encoding="utf-8",
    )
...
def build_index_html(
    *,
    ...
    for row in segment_rows.to_dict(orient="records"):
        ...
        summary_href = f"{segment_name}/model_f_summary.html"
        summary_with_coverage_href = f"{segment_name}/model_f_summary_with_coverage.html"
...
<head>
  ...
  <title>Model F Real-Time Volatility Decomposition</title>
  {dark_mode_style_block()}
</head>
<body>
<main>
  <h1>Model F Real-Time Volatility Decomposition</h1>
  <p>
    This directory contains five segment-specific copies of the usual Model F outputs, split by
  ...
  <ul>
    ...
    <li><code>all_scored_rows_with_volatility.csv</code>: scored Model F rows with attached hourly volatility metadata.</li>
...
def main() -> None:
    ...
    if preferred_hourly_states is not None:
        ...
        if missing_events:
            ...
            raise ValueError(
                "Precomputed hourly volatility table does not cover the filtered Model F event universe. "
    ...
    segment_rows["model_f_summary_html"] = segment_rows["segment_name"].map(
        lambda value: f"{value}/model_f_summary.html"
    )
    segment_rows["model_f_summary_with_coverage_html"] = segment_rows["segment_name"].map(
        lambda value: f"{value}/model_f_summary_with_coverage.html"
    )
    ...
    print(f"Model F real-time volatility decomposition outputs saved to: {output_root}")
