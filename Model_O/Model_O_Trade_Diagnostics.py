...
OUTPUT_FOLDER_NAME = "Model_O_Trade_Diagnostics_Outputs"
DEFAULT_TAKER_FEE_RATE = 0.07
DEFAULT_PRIMARY_STRATEGY_ID = "model_o_exp_lambda_2"
DEFAULT_HAR_RV_QUANTILE = 0.75
DEFAULT_HAR_RV_MIN_TRAIN_HOURS = 168
DEFAULT_HAR_RV_MIN_HISTORY_HOURS = 72
HAR_FEATURES = ["rv_lag_1h", "rv_mean_6h", "rv_mean_24h", "rv_mean_72h"]
HAR_RV_QUANTILE_SWEEP = tuple(value / 100 for value in range(50, 100, 5))
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build Model O fee-aware trade diagnostics: requested trade ledger fields, "
    ...
    parser.add_argument(
        ...
        help="Hourly realized-volatility table used for causal HAR-RV filter forecasts.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--har-rv-quantile",
        type=float,
        default=DEFAULT_HAR_RV_QUANTILE,
        help="Historical realized-volatility quantile threshold for HAR-RV trade activation.",
    )
    parser.add_argument(
        "--har-rv-min-train-hours",
        type=int,
        default=DEFAULT_HAR_RV_MIN_TRAIN_HOURS,
        help="Minimum prior complete hourly observations required before fitting HAR-RV.",
    )
    parser.add_argument(
        "--har-rv-min-history-hours",
        type=int,
        default=DEFAULT_HAR_RV_MIN_HISTORY_HOURS,
        help="Minimum prior realized-volatility observations required before threshold quantiles are usable.",
    )
    parser.add_argument(
        "--disable-har-rv-filter",
        action="store_true",
        help="Skip HAR-RV activation outputs. Baseline diagnostics still run.",
    )
...
def load_hourly_volatility(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Hourly volatility file not found: {path}")

    frame = pd.read_csv(path)
    required = {"forecast_hour_start_utc", "realized_volatility"}
    missing = required - set(frame.columns)
    if missing:
        raise ValueError(f"{path} is missing required columns: {sorted(missing)}")
...
def fit_predict_expanding_har(
    ...
    *,
    ...
    complete_indices = out.dropna(subset=HAR_FEATURES + ["realized_volatility"]).index.to_numpy()
    for idx in complete_indices:
        ...
        out.at[idx, "har_rv_forecast"] = float(max(0.0, x_now @ beta))
    ...
    for quantile in quantiles:
        out[historic_rv_column(quantile)] = prior_realized_volatility.expanding(
            min_periods=min_history_hours
        ).quantile(quantile)
    return out


def attach_har_rv_filter(
    trades: pd.DataFrame,
    har: pd.DataFrame,
    *,
    quantile: float,
    ...
    if threshold_column not in har.columns:
        raise ValueError(f"HAR-RV forecast table is missing {threshold_column}.")

    out = trades.copy()
    out["forecast_hour_start_utc"] = pd.to_datetime(out["timestamp"], utc=True).dt.floor("h")
    keep = [
        "forecast_hour_start_utc",
        "realized_volatility",
        "rolling_percentile_rank",
    ...
    merged["har_rv_threshold_quantile"] = float(quantile)
    merged["har_rv_threshold_label"] = quantile_label(quantile)
    merged["har_rv_threshold"] = pd.to_numeric(merged[threshold_column], errors="coerce")
    merged["har_rv_active"] = (
        pd.to_numeric(merged["har_rv_forecast"], errors="coerce") >= merged["har_rv_threshold"]
    ) & merged["har_rv_threshold"].notna()
    merged["har_rv_filter"] = np.where(merged["har_rv_active"], "HAR-RV active", "HAR-RV inactive")
    return merged


def build_overlap(model_o: pd.DataFrame, model_k: pd.DataFrame, volatility: pd.DataFrame) -> pd.DataFrame:
    required_model_o = {"p_normal", "p_shock", "p_exponential_hybrid"}
    missing = required_model_o - set(model_o.columns)
    if missing:
        raise ValueError(f"Model O raw values are missing required strategy columns: {sorted(missing)}")
    ...
    optional_model_o_cols = ["binance_audit_price", "binance_reference_price", "join_key_used"]
    model_o_cols.extend([col for col in optional_model_o_cols if col in model_o.columns])
...
def build_har_filter_strategy_summary(trades: pd.DataFrame) -> pd.DataFrame:
    rows: List[Dict[str, Any]] = []
    if "har_rv_active" not in trades.columns:
        return pd.DataFrame()

    for strategy_id, strategy_label, _prob_col in STRATEGIES:
        part = trades[trades["strategy_id"] == strategy_id].copy()
        filtered = part[part["har_rv_active"]].copy()
        baseline = summarize_group(part)
        filtered_summary = summarize_group(filtered)
        rows.append(
            {
                "strategy_id": strategy_id,
                "strategy_label": strategy_label,
                "baseline_trade_count": baseline["trade_count"],
                "filtered_trade_count": filtered_summary["trade_count"],
                "trade_keep_rate": (
                    filtered_summary["trade_count"] / baseline["trade_count"] if baseline["trade_count"] else np.nan
                ),
                "baseline_total_net_pnl": baseline["total_net_pnl"],
                "filtered_total_net_pnl": filtered_summary["total_net_pnl"],
                "delta_net_pnl_vs_baseline": filtered_summary["total_net_pnl"] - baseline["total_net_pnl"],
                "baseline_mean_net_pnl": baseline["mean_net_pnl"],
                "filtered_mean_net_pnl": filtered_summary["mean_net_pnl"],
                "baseline_total_fees": baseline["total_fees"],
                "filtered_total_fees": filtered_summary["total_fees"],
                "baseline_roi_after_fees": baseline["roi_after_fees"],
                "filtered_roi_after_fees": filtered_summary["roi_after_fees"],
                "baseline_winrate": baseline["winrate"],
                "filtered_winrate": filtered_summary["winrate"],
                "filtered_active_hour_count": (
                    int(filtered["forecast_hour_start_utc"].nunique()) if "forecast_hour_start_utc" in filtered.columns else 0
                ),
                "filtered_timestamp_count": int(filtered["timestamp"].nunique()) if len(filtered) else 0,
                "filtered_mean_har_rv_forecast": (
                    float(filtered["har_rv_forecast"].mean()) if len(filtered) and "har_rv_forecast" in filtered.columns else np.nan
                ),
                "filtered_mean_realized_volatility": (
                    float(filtered["realized_volatility"].mean())
                    if len(filtered) and "realized_volatility" in filtered.columns
                    else np.nan
                ),
            }
        )
...
def build_har_filter_pnl_timeseries(trades: pd.DataFrame) -> pd.DataFrame:
    if trades.empty or "har_rv_active" not in trades.columns:
        return pd.DataFrame()
    frames: List[pd.DataFrame] = []
    scenarios = [
        ("baseline", "Baseline", trades),
        ("har_rv_filtered", "HAR-RV Filtered", trades[trades["har_rv_active"]]),
    ]
    for scenario_id, scenario_label, part in scenarios:
        if part.empty:
            continue
        grouped = (
        ...
        grouped["scenario_label"] = scenario_label
        grouped["cumulative_net_pnl"] = grouped.groupby("strategy_id")["period_net_pnl"].cumsum()
        grouped["cumulative_gross_pnl"] = grouped.groupby("strategy_id")["period_gross_pnl"].cumsum()
        grouped["cumulative_fees"] = grouped.groupby("strategy_id")["period_fees"].cumsum()
        grouped["cumulative_trades"] = grouped.groupby("strategy_id")["period_trades"].cumsum()
...
def build_har_quantile_sweep_summary(
    trades: pd.DataFrame,
    har: pd.DataFrame,
    quantiles: Sequence[float],
) -> pd.DataFrame:
    if trades.empty or "forecast_hour_start_utc" not in trades.columns:
        return pd.DataFrame()
    ...
    for quantile in quantiles:
        ...
        for strategy_id, strategy_label, _prob_col in STRATEGIES:
            part = trades[trades["strategy_id"] == strategy_id].copy()
            filtered = part[part["forecast_hour_start_utc"].isin(active_hour_set)].copy()
            baseline = summarize_group(part)
            filtered_summary = summarize_group(filtered)
            rows.append(
                {
                    "threshold_label": quantile_label(quantile),
...
def build_har_quantile_sweep_matrix(sweep_summary: pd.DataFrame) -> pd.DataFrame:
...
def build_field_matrix() -> pd.DataFrame:
    rows = [
        ...
        ("market / strike / expiry", "market, strike, expiry", "Model O eval raw values", True, "Contract ticker, strike, and event expiry."),
        ...
        (
            "HAR-RV filter",
            "har_rv_active, har_rv_forecast, har_rv_threshold",
            "Causal hourly HAR-RV forecast",
            True,
            "Active when the HAR-RV forecast exceeds the selected historical realized-volatility quantile built from prior hours only.",
        ),
        ("q_shock", "q_shock", "Model O shock classifier", True, "Predicted shock probability."),
        (
            "model disagreement",
        ...
        (
            ...
            False,
            "Current historical Model_K/Model_O files do not store bid/ask/spread/liquidity; columns are kept as unavailable.",
        ),
    ]
...
def ledger_columns(trades: pd.DataFrame) -> List[str]:
    cols = [
        ...
        "har_rv_forecast",
        "har_rv_threshold",
        "har_rv_threshold_quantile",
        "har_rv_threshold_label",
        "har_train_rows",
        "rv_lag_1h",
        "rv_mean_6h",
        "rv_mean_24h",
...
def har_filter_pnl_figure(timeseries: pd.DataFrame, *, primary_strategy_id: str) -> Any:
    fig = go.Figure()
...
def har_quantile_sweep_figure(sweep_summary: pd.DataFrame) -> Any:
    fig = make_subplots(
        rows=1,
        cols=2,
...
def har_dashboard_cards(har_summary: pd.DataFrame, *, primary_strategy_id: str) -> str:
    if har_summary.empty:
        return ""
    primary = har_summary[har_summary["strategy_id"] == primary_strategy_id]
    row = primary.iloc[0] if not primary.empty else har_summary.iloc[0]
    cards = [
        ("HAR Filtered Net PnL", format_number(row["filtered_total_net_pnl"], digits=3, signed=True)),
        ("HAR Trades Kept", format_number(row["trade_keep_rate"], percent=True)),
        ("HAR PnL / Trade", format_number(row["filtered_mean_net_pnl"], digits=4, signed=True)),
        ("HAR Active Hours", format_number(row["filtered_active_hour_count"], digits=0)),
    ]
    return "".join(
        f"<section class='metric-card'><span>{html.escape(label)}</span><strong>{html.escape(value)}</strong></section>"
        for label, value in cards
...
def build_dashboard_html(
    *,
    ...
    har_filter_strategy_summary: pd.DataFrame,
    har_filter_pnl_timeseries: pd.DataFrame,
    har_quantile_sweep_matrix: pd.DataFrame,
    har_quantile_sweep_summary: pd.DataFrame,
    ...
    if not har_filter_pnl_timeseries.empty:
        figures.append(har_filter_pnl_figure(har_filter_pnl_timeseries, primary_strategy_id=primary_strategy_id))
    if not har_quantile_sweep_summary.empty:
        figures.append(har_quantile_sweep_figure(har_quantile_sweep_summary))
...
<head>
  ...
  <title>Model O Trade Diagnostics</title>
  {dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    <p class="eyebrow">Model O Diagnostics</p>
    ...
    <p class="lead">
      ...
      <code>{round_up_to_cent}</code>. The HAR-RV filter is causal: features and thresholds use prior hourly
      realized volatility only.
    </p>
    <div class="cards">{dashboard_cards(strategy_summary, primary_strategy_id=primary_strategy_id)}</div>
    <div class="cards">{har_dashboard_cards(har_filter_strategy_summary, primary_strategy_id=primary_strategy_id)}</div>
  </section>
  ...
  <h2>HAR-RV Filter Strategy Summary</h2>
  <section class="panel"><div class="table-wrap">{dataframe_to_html_table(format_table(har_filter_strategy_summary))}</div></section>

  <h2>HAR-RV q50-q95 Sweep Matrix</h2>
  <section class="panel"><div class="table-wrap">{dataframe_to_html_table(format_table(har_quantile_sweep_matrix))}</div></section>
...
def write_readme(
    ...
    *,
    ...
    text = f"""Model O Trade Diagnostics
...
def main() -> None:
    ...
    if not 0.0 < args.har_rv_quantile < 1.0:
        raise ValueError("--har-rv-quantile must be between 0 and 1.")
    if args.har_rv_min_train_hours < 1:
        raise ValueError("--har-rv-min-train-hours must be positive.")
    if args.har_rv_min_history_hours < 1:
        raise ValueError("--har-rv-min-history-hours must be positive.")
    ...
    overlap = build_overlap(model_o, model_k, volatility)
    ...
    har = pd.DataFrame()
    if args.disable_har_rv_filter:
        trades["forecast_hour_start_utc"] = pd.to_datetime(trades["timestamp"], utc=True).dt.floor("h")
        trades["har_rv_active"] = False
        trades["har_rv_filter"] = "HAR-RV disabled"
        trades["har_rv_forecast"] = np.nan
        trades["har_rv_threshold"] = np.nan
        trades["har_rv_threshold_quantile"] = float(args.har_rv_quantile)
        trades["har_rv_threshold_label"] = quantile_label(args.har_rv_quantile)
    else:
        quantiles = tuple(sorted(set([*HAR_RV_QUANTILE_SWEEP, float(args.har_rv_quantile)])))
        hourly_volatility = load_hourly_volatility(args.hourly_volatility.resolve())
        har = fit_predict_expanding_har(
            hourly_volatility,
            min_train_hours=args.har_rv_min_train_hours,
            min_history_hours=args.har_rv_min_history_hours,
            quantiles=quantiles,
        )
        trades = attach_har_rv_filter(trades, har, quantile=float(args.har_rv_quantile))

    field_matrix = build_field_matrix()
    strategy_summary = build_strategy_summary(trades)
    har_filter_strategy_summary = build_har_filter_strategy_summary(trades)
    timeseries = build_pnl_timeseries(trades)
    har_filter_pnl_timeseries = build_har_filter_pnl_timeseries(trades)
    ...
    har_quantile_sweep_summary = (
        build_har_quantile_sweep_summary(trades, har, HAR_RV_QUANTILE_SWEEP) if not har.empty else pd.DataFrame()
    )
    har_quantile_sweep_matrix = build_har_quantile_sweep_matrix(har_quantile_sweep_summary)

    trades[ledger_columns(trades)].to_csv(output_dir / "trade_diagnostics_ledger.csv", index=False)
    trades.loc[trades["har_rv_active"], ledger_columns(trades)].to_csv(
        output_dir / "har_rv_active_trade_ledger.csv", index=False
    )
    ...
    har_filter_strategy_summary.to_csv(output_dir / "har_rv_filter_strategy_summary.csv", index=False)
    timeseries.to_csv(output_dir / "pnl_timeseries.csv", index=False)
    har_filter_pnl_timeseries.to_csv(output_dir / "har_rv_filter_pnl_timeseries.csv", index=False)
    ...
    har_quantile_sweep_summary.to_csv(output_dir / "har_rv_quantile_sweep_strategy_summary.csv", index=False)
    har_quantile_sweep_matrix.to_csv(output_dir / "har_rv_quantile_sweep_matrix.csv", index=False)

    dashboard_html = build_dashboard_html(
        field_matrix=field_matrix,
        strategy_summary=strategy_summary,
        har_filter_strategy_summary=har_filter_strategy_summary,
        har_filter_pnl_timeseries=har_filter_pnl_timeseries,
        har_quantile_sweep_matrix=har_quantile_sweep_matrix,
        har_quantile_sweep_summary=har_quantile_sweep_summary,
    ...
    if not har_filter_strategy_summary.empty:
        har_display = har_filter_strategy_summary[
            [
                "strategy_label",
                "filtered_trade_count",
                "trade_keep_rate",
        ...
        print("\nHAR-RV filtered strategy summary:")
        print(har_display.to_string(index=False))
