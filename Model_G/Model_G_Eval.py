...
EPS = 1e-15
ASSUMPTIONS = (
    "Model G forecast values are interpreted as YES probabilities. CSV values greater than 1 and no greater than 100 "
    ...
    "Model G probabilities include an isotonic regression layer fit only on high-volatility-extreme historical minutes "
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate Model G BTC hourly contract probabilities against official "
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "model_G_Evals_Outputs",
        help="Directory for Model G evaluation output files.",
    )
    parser.add_argument(
        "--skip-binance-audit",
        action="store_true",
...
def write_individual_metric_files(
    *,
    ...
    (output_dir / "model_g_assumptions.txt").write_text("\n".join(ASSUMPTIONS) + "\n", encoding="utf-8")
...
def svg_line_chart(
    ...
    *,
    ...
    <svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(title)}">
      ...
      {circles}
      <text x="{plot_left}" y="{height - 12}" class="axis-label">Mean Model G probability</text>
...
def svg_histogram(
    ...
    *,
    ...
    <svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(title)}">
      ...
      <line x1="{plot_left}" y1="{plot_top}" x2="{plot_left}" y2="{plot_bottom}" class="axis" />
      {''.join(bars)}
      <text x="{plot_left}" y="{height - 12}" class="axis-label">Model G probability</text>
...
def plotly_report_charts(raw: pd.DataFrame, calibration: pd.DataFrame) -> str:
    ...
    fig = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=("Calibration", "Model G Probability Distribution"),
    ...
    fig.update_xaxes(range=[0, 1], title_text="Mean Model G probability", row=1, col=1)
    fig.update_yaxes(range=[0, 1], title_text="Observed frequency", row=1, col=1)
    fig.update_xaxes(range=[0, 1], title_text="Model G probability", row=1, col=2)
...
def fallback_report_charts(raw: pd.DataFrame, calibration: pd.DataFrame) -> str:
    ...
    return (
        ...
        + svg_histogram(raw, value_col="p_kalshi", title="Model G Probability Distribution")
        + "</div>"
    )
...
<main>
  ...
  <p>
    Forecasts are minute-level Model G YES probabilities from the hourly forecast files. Reality is the
...
def main() -> None:
    ...
    summary_path = output_dir / "model_g_summary.html"
    ...
    summary_with_coverage_path = output_dir / "model_g_summary_with_coverage.html"
    ...
    print(f"Model G outputs saved to: {output_dir}")
