...
USER_AGENT = "model-g-high-vol-extreme-garch-studentt-builder/1.0"

OUTPUT_FOLDER_NAME = "Model_G_Output_Raw_Data"
EPS = 1e-12
MIN_VARIANCE = 1e-12
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build Model G raw hourly forecast files by combining settlement-defined Kalshi "
    ...
    parser.add_argument(
        ...
        type=int,
        default=240,
        help="Minimum number of regime-filtered 1-minute returns required before a Model G fit is allowed.",
    )
    parser.add_argument(
        "--isotonic-mc-trials",
        type=int,
...
def select_regime_training_sample(
    ...
    *,
    ...
    ][["price_index", "datetime_utc", "hour_start_utc", "spot_open", "log_return"]].copy()
...
def fit_isotonic_calibration_layer(
    ...
    *,
    ...
    if training_frame.empty:
        raise RuntimeError("No synthetic calibration examples were available for the Model G isotonic layer.")
    ...
    for schedule_idx, forecast in enumerate(schedule.to_dict(orient="records"), start=1):
        ...
        if need_refit:
            ...
            if regime_sample.n_returns < args.min_regime_observations:
                raise RuntimeError(
                    "Insufficient regime-filtered history for Model G fit at "
        ...
        fitted_forecasts.append(
            {
                ...
                "ATM_price": calibrated_probability_from_terminals(terminals, forecast["ATM_strike"], calibration_layer),
                "OTM+1_market_ticker": forecast["OTM+1_market_ticker"],
                "OTM+1_strike": forecast["OTM+1_strike"],
                "OTM+1_price": calibrated_probability_from_terminals(terminals, forecast["OTM+1_strike"], calibration_layer),
                "OTM-1_market_ticker": forecast["OTM-1_market_ticker"],
                "OTM-1_strike": forecast["OTM-1_strike"],
                "OTM-1_price": calibrated_probability_from_terminals(terminals, forecast["OTM-1_strike"], calibration_layer),
            }
