...
OUTPUT_FOLDER_NAME = "Model_H_Isotonic_Diagnostics_Outputs"
EPS = 1e-12
...
def default_diagnostics_dir(root: Path) -> Path:
    return root / "Model_H" / OUTPUT_FOLDER_NAME


def default_volatility_segments_csv(root: Path) -> Optional[Path]:
    candidates = [
        root
        / "Model_H_Volatility_Decomposition_RT"
        / "Model_H_Volatility_Decomposition_RT_outputs"
        / "hourly_market_volatility_segments.csv",
        root
        / "Model_H_Volatility_Results_Dashboard"
        / "Model_H_Volatility_Dashboards_outputs"
        / "hourly_market_volatility_segments.csv",
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Model H isotonic calibration diagnostics from raw-vs-calibrated outputs."
    )
    parser.add_argument(
        "--model-h-eval-dir",
        type=Path,
        default=default_eval_dir(REPO_ROOT),
        help="Directory containing Model_H_Eval.py outputs, especially raw_values.csv.",
    )
    parser.add_argument("--model-f-eval-dir", dest="model_h_eval_dir", type=Path, help=argparse.SUPPRESS)
    parser.add_argument(
        "--isotonic-diagnostics-dir",
        type=Path,
        default=default_diagnostics_dir(REPO_ROOT),
        help="Directory containing Model_H.py isotonic diagnostic CSVs.",
    )
    parser.add_argument(
...
def load_backtest_frame(eval_dir: Path, warnings: List[str]) -> pd.DataFrame:
    ...
    if not required.issubset(raw.columns):
        warnings.append(
            ...
            "Rerun Model_H.py and Model_H_Eval.py before using the backtest raw-vs-calibrated diagnostics."
        )
        return pd.DataFrame()
    ...
    if out.empty:
        warnings.append(
            ...
            "This usually means the copied seed outputs have not been regenerated with Model_H.py yet."
        )
    return out
    ...
    *,
    ...
    hve_sections = [
        section(
            ...
            "When Model_H.py has been rerun with the new instrumentation, the training comparison also filters "
...
<head>
  ...
  <title>Model H Isotonic Diagnostics</title>
  {dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    <h1>Model H Isotonic Diagnostics</h1>
    <p>
      This dashboard compares pre-isotonic raw probabilities against calibrated Model H probabilities,
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Model H High-Volatility-Extreme Isotonic Diagnostics</title>
  {dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    <h1>Model H HVE Isotonic Diagnostics</h1>
    <p>
      This standalone dashboard isolates high-volatility-extreme rows and compares pre-isotonic raw
      probabilities against calibrated Model H probabilities inside that slice.
...
def main() -> None:
    args = parse_args()
    eval_dir = args.model_h_eval_dir.resolve()
    ...
    if training.empty:
        warnings.append(
            "isotonic_training_diagnostics.csv is missing or empty. Rerun Model_H.py to save isotonic training diagnostics."
        )
    else:
    ...
    if not backtest.empty and hve_backtest.empty:
        warnings.append(
            ...
            "Run the Model_H volatility decomposition/dashboard first, or provide --volatility-segments-csv."
        )
    if not training.empty and hve_training.empty:
        warnings.append(
            "No high-volatility-extreme isotonic training examples were available. "
            "Rerun Model_H.py with the latest instrumentation, or check whether the selected training regime contains HVE minutes."
        )
    ...
    print(f"Model H isotonic diagnostics written to: {output_dir}")
    print(f"Model H high-volatility-extreme isotonic dashboard written to: {hve_output_dir / 'index.html'}")
