...
OUTPUT_FOLDER_NAME = "Model_H_Stats_Dashboard_Outputs"
...
def default_model_h_eval_dir(root: Path) -> Path:
    return root / "Model_H" / "model_H_Evals_Outputs"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build dark-mode internal and comparative dashboards for Model H using the "
            "evaluation output folders."
        )
    )
    parser.add_argument(
        "--model-h-eval-dir",
        dest="model_h_eval_dir",
        type=Path,
        default=default_model_h_eval_dir(REPO_ROOT),
        help="Directory containing Model_H evaluation outputs.",
    )
    parser.add_argument("--model-g-eval-dir", dest="model_h_eval_dir", type=Path, help=argparse.SUPPRESS)
    parser.add_argument(
        "--model-k-eval-dir",
        type=Path,
...
def internal_charts_html(bundle: Dict[str, Any], pit: np.ndarray) -> str:
    ...
    fig_top = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=("Calibration Curve", "Model H Probability Distribution"),
    ...
    fig_top.update_xaxes(range=[0, 1], title_text="Mean Model H probability", row=1, col=1)
    fig_top.update_yaxes(range=[0, 1], title_text="Observed frequency", row=1, col=1)
    fig_top.update_xaxes(range=[0, 1], title_text="Model H probability", row=1, col=2)
...
def comparison_charts_html(bundle_a: Dict[str, Any], bundle_k: Dict[str, Any]) -> str:
    ...
    fig_bottom.add_trace(
        go.Bar(x=labels, y=[float(dec_a[c]) for c in cols], marker_color=COLOR_A, name="Model H"),
        row=1,
        col=1,
    )
    fig_bottom.add_trace(
...
<head>
  ...
  <title>Model H Internal Dashboard</title>
  {dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    ...
    <h1>Model H Probabilistic Diagnostics</h1>
    <p class="lead">
      This dashboard summarizes Model H as a probabilistic forecaster on its own scored output universe,
...
def overview_comparison_table(bundle_a: Dict[str, Any], bundle_k: Dict[str, Any]) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [
            {"metric": metric, "model_h": left, "model_k": right, "delta_g_minus_k": left - right}
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Model H vs Model K Dashboard</title>
  {dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    ...
    <h1>Model H vs Model K</h1>
    <p class="lead">
      This dashboard compares Model H and Model K on the exact overlapping scored-row universe,
    ...
    <div class="hero-pills">
      ...
      <span class="pill">Model H Brier: {format_number(oa["brier_score"])}</span>
...
def main() -> None:
    ...
    model_h_eval_dir = args.model_h_eval_dir.resolve()
    model_k_eval_dir = args.model_k_eval_dir.resolve()

    loaded_b = load_eval_dir(model_h_eval_dir)
    ...
    internal_notes = [
        f"Source directory: {model_h_eval_dir}",
        f"Randomized PIT seed: {args.pit_seed}",
        "Internal dashboard uses the full Model H scored universe currently present in raw_values.csv.",
    ]
    internal_html = build_internal_dashboard_html(bundle_a_internal, pit_values, internal_notes)
    (output_dir / "model_h_internal_dashboard.html").write_text(internal_html, encoding="utf-8")

    aligned_b_raw, aligned_k_raw = intersect_raw_rows(loaded_b["raw"], loaded_k["raw"])
    if aligned_b_raw.empty or aligned_k_raw.empty:
        raise RuntimeError("No overlapping scored rows were found between Model H and Model K.")
    ...
    comparative_notes = [
        f"Model H source directory: {model_h_eval_dir}",
    ...
    (output_dir / "model_h_vs_model_k_dashboard.html").write_text(comparative_html, encoding="utf-8")

    print(f"Internal dashboard: {output_dir / 'model_h_internal_dashboard.html'}")
    print(f"Comparative dashboard: {output_dir / 'model_h_vs_model_k_dashboard.html'}")
