...
EPS = 1e-15
ASSUMPTIONS = (
    "Model H forecast values are interpreted as YES probabilities. CSV values greater than 1 and no greater than 100 "
    ...
    "Model H probabilities include an isotonic regression layer fit only on high-volatility-extreme historical minutes "
    "outside the Kalshi settlement/pricing backtest range.",
    "When present, Model H raw_probability diagnostics are the pre-isotonic Monte Carlo YES probabilities and "
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate Model H BTC hourly contract probabilities against official "
    ...
    parser.add_argument(
        ...
        help="Folder containing Model_H.py hourly raw forecast CSV outputs.",
    )
    parser.add_argument(
        "--settlement-csv",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "model_H_Evals_Outputs",
        help="Directory for Model H evaluation output files.",
    )
    parser.add_argument(
        "--skip-binance-audit",
        action="store_true",
...
def write_individual_metric_files(
    *,
    ...
    (output_dir / "model_h_assumptions.txt").write_text("\n".join(ASSUMPTIONS) + "\n", encoding="utf-8")
...
def svg_line_chart(
    ...
    *,
    ...
    <svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(title)}">
      ...
      {circles}
      <text x="{plot_left}" y="{height - 12}" class="axis-label">Mean Model H probability</text>
...
def svg_histogram(
    ...
    *,
    ...
    <svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(title)}">
      ...
      <line x1="{plot_left}" y1="{plot_top}" x2="{plot_left}" y2="{plot_bottom}" class="axis" />
      {''.join(bars)}
      <text x="{plot_left}" y="{height - 12}" class="axis-label">Model H probability</text>
...
def plotly_report_charts(raw: pd.DataFrame, calibration: pd.DataFrame) -> str:
    ...
    fig = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=("Calibration", "Model H Probability Distribution"),
    ...
    fig.update_xaxes(range=[0, 1], title_text="Mean Model H probability", row=1, col=1)
    fig.update_yaxes(range=[0, 1], title_text="Observed frequency", row=1, col=1)
    fig.update_xaxes(range=[0, 1], title_text="Model H probability", row=1, col=2)
...
def fallback_report_charts(raw: pd.DataFrame, calibration: pd.DataFrame) -> str:
    ...
    return (
        ...
        + svg_histogram(raw, value_col="p_kalshi", title="Model H Probability Distribution")
        + "</div>"
    )
...
<body>
<main>
  <h1>Model H: Probability Evaluation</h1>
  <p>
    Forecasts are minute-level Model H YES probabilities from the hourly forecast files. Reality is the
  ...
  <h2>Output Files</h2>
  <p class="note">
    Individual CSV outputs are saved next to this report in <code>model_H_Evals_Outputs</code>:
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Model H Market Minute Bucket Summary</title>
  <style>
    :root {{
      color-scheme: dark;
      --bg: #07111f;
...
<body>
<main>
  <h1>Model H: Market Minute Buckets</h1>
  <p>
...
def main() -> None:
    ...
    summary_path = output_dir / "model_h_summary.html"
    ...
    summary_with_coverage_path = output_dir / "model_h_summary_with_coverage.html"
    ...
    print(f"Model H outputs saved to: {output_dir}")
