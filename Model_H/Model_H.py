...
USER_AGENT = "model-h-isotonic-diagnostics-garch-studentt-builder/1.0"

OUTPUT_FOLDER_NAME = "Model_H_Output_Raw_Data"
DIAGNOSTICS_FOLDER_NAME = "Model_H_Isotonic_Diagnostics_Outputs"
EPS = 1e-12
MIN_VARIANCE = 1e-12
...
VOLATILITY_DIAGNOSTIC_COLUMNS: Tuple[str, ...] = (
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build Model H raw hourly forecast files by combining settlement-defined Kalshi "
    ...
    parser.add_argument(
        "--diagnostics-dir",
        type=Path,
        default=Path(__file__).resolve().parent / DIAGNOSTICS_FOLDER_NAME,
        help=(
            "Directory where Model H isotonic diagnostics will be written. "
            "This includes whole-sample and high-volatility-extreme forecast diagnostics, "
            "plus synthetic isotonic training diagnostics by refit."
        ),
    )
    parser.add_argument(
        "--symbol",
    ...
    parser.add_argument(
        ...
        default="high_volatility_extreme",
        help=(
    ...
    parser.add_argument(
        ...
        type=int,
        default=240,
        help="Minimum number of regime-filtered 1-minute returns required before a Model H fit is allowed.",
    )
    parser.add_argument(
        "--isotonic-mc-trials",
        type=int,
...
def select_regime_training_sample(
    ...
    *,
    ...
    sample_columns.extend(column for column in VOLATILITY_DIAGNOSTIC_COLUMNS if column in prices.columns)
    eligible = prices[
...
def calibrated_probability_series(
    raw_probabilities: pd.Series,
    calibration_layer: IsotonicCalibrationLayer,
) -> pd.Series:
    values = pd.to_numeric(raw_probabilities, errors="coerce")
    calibrated = pd.Series(np.nan, index=values.index, dtype=float)
    valid = values.notna()
    if valid.any():
        calibrated.loc[valid] = np.clip(
            np.interp(
                values.loc[valid].to_numpy(dtype=float),
                calibration_layer.x_thresholds,
                calibration_layer.y_thresholds,
            ),
            0.0,
            1.0,
        )
...
def build_isotonic_training_frame(
    ...
    *,
    ...
    for row in rows.to_dict(orient="records"):
        ...
        for column in VOLATILITY_DIAGNOSTIC_COLUMNS:
            if column in row:
            ...
            calibration_rows.append(
                {
                    ...
                    "forecast_datetime_utc": row["forecast_dt"],
                    "training_hour_start_utc": row["hour_start_utc"],
                    "horizon_steps": int(row["horizon_steps"]),
                    "current_spot": float(row["current_spot"]),
                    ...
                    **{column: row[column] for column in VOLATILITY_DIAGNOSTIC_COLUMNS if column in row},
                }
            )

    out = pd.DataFrame(calibration_rows)
    if out.empty:
        return pd.DataFrame(columns=diagnostic_columns)
    return out
...
def fit_isotonic_calibration_layer(
    ...
    *,
    ...
    if training_frame.empty:
        raise RuntimeError("No synthetic calibration examples were available for the Model H isotonic layer.")
...
def high_volatility_extreme_slice(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.empty or "is_high_volatility_extreme" not in frame.columns:
        return frame.iloc[0:0].copy()
    return frame[coerce_bool(frame["is_high_volatility_extreme"])].copy()


def isotonic_curve_frame(calibration_layer: IsotonicCalibrationLayer, *, refit_id: int) -> pd.DataFrame:
    return pd.DataFrame(
        {
            "refit_id": refit_id,
            "curve_point_index": np.arange(len(calibration_layer.x_thresholds), dtype=int),
...
def add_isotonic_training_diagnostics(
    training_frame: pd.DataFrame,
    calibration_layer: IsotonicCalibrationLayer,
    *,
    refit_id: int,
    ...
    out["calibrated_probability"] = calibrated_probability_series(out["raw_probability"], calibration_layer)
    out["calibration_shift"] = out["calibrated_probability"] - out["raw_probability"]
    out["refit_id"] = refit_id
    ordered = [
        "refit_id",
        "raw_probability",
...
def write_isotonic_diagnostics(
    ...
    *,
    ...
    forecast_diagnostics.to_csv(diagnostics_dir / "forecast_probability_diagnostics.csv", index=False)
    high_volatility_extreme_slice(forecast_diagnostics).to_csv(
        diagnostics_dir / "forecast_probability_diagnostics_high_volatility_extreme.csv",
        index=False,
    )
    training_diagnostics.to_csv(diagnostics_dir / "isotonic_training_diagnostics.csv", index=False)
    high_volatility_extreme_slice(training_diagnostics).to_csv(
        diagnostics_dir / "isotonic_training_diagnostics_high_volatility_extreme.csv",
        index=False,
    )
    curve_points.to_csv(diagnostics_dir / "isotonic_curve_points.csv", index=False)
    refit_summary.to_csv(diagnostics_dir / "isotonic_refit_summary.csv", index=False)
...
def main() -> None:
    ...
    forecast_diagnostic_rows: List[Dict[str, Any]] = []
    isotonic_training_diagnostics: List[pd.DataFrame] = []
    isotonic_curve_points: List[pd.DataFrame] = []
    isotonic_refit_summaries: List[Dict[str, Any]] = []
    ...
    for schedule_idx, forecast in enumerate(schedule.to_dict(orient="records"), start=1):
        ...
        if need_refit:
            ...
            if regime_sample.n_returns < args.min_regime_observations:
                raise RuntimeError(
                    "Insufficient regime-filtered history for Model H fit at "
            ...
            refit_id += 1
            training_diagnostics = add_isotonic_training_diagnostics(
                isotonic_training,
                calibration_layer,
                refit_id=refit_id,
            )
            isotonic_training_diagnostics.append(training_diagnostics)
            isotonic_curve_points.append(isotonic_curve_frame(calibration_layer, refit_id=refit_id))
            ...
            isotonic_refit_summaries.append(
                {
                    "refit_id": refit_id,
                    "schedule_index": schedule_idx,
                    "event_ticker": forecast["Event"],
                    "forecast_datetime_utc": forecast_dt,
                    "training_segment": training_segment["label"],
                    "regime_returns": regime_sample.n_returns,
                    "regime_blocks": regime_sample.n_blocks,
                    "regime_hours": regime_sample.n_hours,
                    "regime_lookback_start_utc": regime_sample.lookback_start_utc,
                    "regime_lookback_end_utc": regime_sample.lookback_end_utc,
                    "isotonic_examples": calibration_layer.n_samples,
                    "isotonic_minutes": calibration_layer.n_minutes,
                    "isotonic_curve_points": len(calibration_layer.x_thresholds),
                    "raw_probability_min": float(training_diagnostics["raw_probability"].min()),
                    "raw_probability_max": float(training_diagnostics["raw_probability"].max()),
                    "calibrated_probability_min": float(training_diagnostics["calibrated_probability"].min()),
                    "calibrated_probability_max": float(training_diagnostics["calibrated_probability"].max()),
            ...
            print(
                f"[refit {refit_id} {schedule_idx}/{total}] {forecast['Event']} {forecast_dt.isoformat()} "
        ...
        price_row = prices.iloc[int(current_price_index)]
        volatility_metadata = {
            column: price_row[column]
            for column in VOLATILITY_DIAGNOSTIC_COLUMNS
            if column in price_row.index
        }
        ...
        for label, ticker_col, strike_col in CONTRACT_SPECS:
            raw_probability, calibrated = raw_and_calibrated_probability_from_terminals(
                terminals,
                forecast[strike_col],
                calibration_layer,
            )
            forecast_row[ticker_col] = forecast[ticker_col]
            forecast_row[strike_col] = forecast[strike_col]
            forecast_row[f"{label}_raw_price"] = raw_probability
            forecast_row[f"{label}_calibrated_price"] = calibrated
            forecast_row[f"{label}_price"] = calibrated
            forecast_diagnostic_rows.append(
                {
                    "event_ticker": forecast["Event"],
                    "hour_start_utc": forecast["hour_start_utc"],
                    "forecast_datetime_utc": forecast["forecast_datetime_utc"],
                    "minute_number": int(forecast["minute_number"]),
                    "minutes_to_settlement": int(forecast["steps_to_expiry"]),
                    "contract_label": label,
                    "market_ticker": forecast[ticker_col],
                    "strike": forecast[strike_col],
                    "raw_probability": raw_probability,
                    "calibrated_probability": calibrated,
                    "calibration_shift": None
                    if raw_probability is None or calibrated is None
                    else float(calibrated) - float(raw_probability),
                    "minute_price_index": int(current_price_index),
                    "spot_open": float(spots[current_price_index]),
                    "next_variance": fit_state.next_variance,
                    "refit_id": refit_id,
                    **volatility_metadata,
                }
            )

        fitted_forecasts.append(forecast_row)
    ...
    forecast_diagnostics = pd.DataFrame(forecast_diagnostic_rows)
    training_diagnostics = (
        pd.concat(isotonic_training_diagnostics, ignore_index=True)
        if isotonic_training_diagnostics
        else pd.DataFrame()
    )
    curve_points = (
        pd.concat(isotonic_curve_points, ignore_index=True)
        if isotonic_curve_points
        else pd.DataFrame()
    )
    refit_summary = pd.DataFrame(isotonic_refit_summaries)
    write_isotonic_diagnostics(
    ...
    print(f"Diagnostics directory: {args.diagnostics_dir.resolve()}")
    ...
    print(f"Forecast diagnostic rows written: {len(forecast_diagnostics)}")
    print(f"Isotonic training diagnostic rows written: {len(training_diagnostics)}")
