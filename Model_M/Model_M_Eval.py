...
EPS = 1e-15
ASSUMPTIONS = (
    "Model M forecast values are interpreted as YES probabilities. CSV values greater than 1 and no greater than 100 "
    ...
    "Model M probabilities include an isotonic regression layer fit only on high-volatility-extreme historical minutes "
    "outside the Kalshi settlement/pricing backtest range.",
    "When present, Model M raw_probability diagnostics are the pre-isotonic Monte Carlo YES probabilities and "
    "calibrated_probability diagnostics are the post-isotonic probabilities used for scoring.",
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate Model M BTC hourly contract probabilities against official "
            "Kalshi settlement outcomes."
        )
    )
    parser.add_argument(
        "--kalshi-price-dir",
        type=Path,
        default=root / "Model_M" / "Model_M_Output_Raw_Data",
        help="Folder containing Model_M.py hourly raw forecast CSV outputs.",
    )
    parser.add_argument(
        "--settlement-csv",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "model_M_Evals_Outputs",
        help="Directory for Model M evaluation output files.",
    )
    parser.add_argument(
        "--skip-binance-audit",
        action="store_true",
...
def load_kalshi_price_outputs(price_dir: Path) -> pd.DataFrame:
    ...
    for label in CONTRACT_LABELS:
        ...
        raw_price_col = f"{label}_raw_price"
        calibrated_price_col = f"{label}_calibrated_price"
        ...
        for optional_col in [raw_price_col, calibrated_price_col, "refit_id"]:
            if optional_col in wide.columns:
        ...
        rename_cols = {
            ...
            raw_price_col: "p_raw_pre_isotonic_raw",
            calibrated_price_col: "p_calibrated_isotonic_raw",
        }
        ...
        if "p_raw_pre_isotonic_raw" in part.columns:
            part["p_raw_pre_isotonic"], _raw_scale = normalize_kalshi_probability(part["p_raw_pre_isotonic_raw"])
        else:
            part["p_raw_pre_isotonic"] = np.nan
        if "p_calibrated_isotonic_raw" in part.columns:
            part["p_calibrated_isotonic"], _calibrated_scale = normalize_kalshi_probability(
                part["p_calibrated_isotonic_raw"]
            )
        else:
            part["p_calibrated_isotonic"] = part["p_kalshi"]
    ...
    out["p_raw_pre_isotonic"] = pd.to_numeric(out["p_raw_pre_isotonic"], errors="coerce")
    out["p_calibrated_isotonic"] = pd.to_numeric(out["p_calibrated_isotonic"], errors="coerce")
    ...
    out["p_raw_pre_isotonic"] = out["p_raw_pre_isotonic"].clip(0.0, 1.0)
    out["p_calibrated_isotonic"] = out["p_calibrated_isotonic"].clip(0.0, 1.0)
...
def build_isotonic_forecast_diagnostics(df: pd.DataFrame) -> pd.DataFrame:
    diagnostic_columns = [
        "event_ticker",
        "forecast_datetime_utc",
        "minute_number",
    ...
    available = [column for column in diagnostic_columns if column in df.columns]
    out = df[available].copy()
    if "p_raw_pre_isotonic" not in out.columns:
        out["p_raw_pre_isotonic"] = np.nan
    if "p_calibrated_isotonic" not in out.columns:
        out["p_calibrated_isotonic"] = out["p_kalshi"]

    out = out.rename(
        columns={
            "p_raw_pre_isotonic": "raw_probability",
            "p_calibrated_isotonic": "calibrated_probability",
            "p_kalshi": "scored_probability",
        }
    )
    out["calibration_shift"] = out["calibrated_probability"] - out["raw_probability"]
    out["absolute_calibration_shift"] = out["calibration_shift"].abs()
    if "outcome" in out.columns:
        out["raw_forecast_error"] = out["raw_probability"] - out["outcome"]
        out["calibrated_forecast_error"] = out["calibrated_probability"] - out["outcome"]
        out["raw_squared_error"] = out["raw_forecast_error"] ** 2
        out["calibrated_squared_error"] = out["calibrated_forecast_error"] ** 2
        out["brier_delta_calibrated_minus_raw"] = out["calibrated_squared_error"] - out["raw_squared_error"]
    return out


def isotonic_diagnostic_summary_row(name: str, frame: pd.DataFrame) -> Dict[str, Any]:
    available = frame.dropna(subset=["raw_probability", "calibrated_probability"]).copy()
    if available.empty:
        return {
            "segment": name,
            "n_forecasts": int(len(frame)),
            "n_with_raw_probability": int(frame["raw_probability"].notna().sum()) if "raw_probability" in frame.columns else 0,
            "n_with_calibrated_probability": int(frame["calibrated_probability"].notna().sum())
            if "calibrated_probability" in frame.columns
            else 0,
            "mean_raw_probability": np.nan,
            "mean_calibrated_probability": np.nan,
            "mean_calibration_shift": np.nan,
            "mean_absolute_calibration_shift": np.nan,
            "raw_brier_score": np.nan,
            "calibrated_brier_score": np.nan,
            "brier_delta_calibrated_minus_raw": np.nan,
            "lower_tail_n_raw_lte_0_10": 0,
            "lower_tail_mean_raw_probability": np.nan,
            "lower_tail_mean_calibrated_probability": np.nan,
            "lower_tail_observed_frequency": np.nan,
        }

    lower_tail = available[available["raw_probability"] <= 0.10].copy()
    return {
        "segment": name,
        "n_forecasts": int(len(frame)),
        "n_with_raw_probability": int(frame["raw_probability"].notna().sum()),
        "n_with_calibrated_probability": int(frame["calibrated_probability"].notna().sum()),
        "mean_raw_probability": float(available["raw_probability"].mean()),
        "mean_calibrated_probability": float(available["calibrated_probability"].mean()),
        "mean_calibration_shift": float(available["calibration_shift"].mean()),
        "mean_absolute_calibration_shift": float(available["absolute_calibration_shift"].mean()),
        "raw_brier_score": float(available["raw_squared_error"].mean())
        if "raw_squared_error" in available.columns
        else np.nan,
        "calibrated_brier_score": float(available["calibrated_squared_error"].mean())
        if "calibrated_squared_error" in available.columns
        else np.nan,
        "brier_delta_calibrated_minus_raw": float(available["brier_delta_calibrated_minus_raw"].mean())
        if "brier_delta_calibrated_minus_raw" in available.columns
        else np.nan,
        "lower_tail_n_raw_lte_0_10": int(len(lower_tail)),
        "lower_tail_mean_raw_probability": float(lower_tail["raw_probability"].mean()) if len(lower_tail) else np.nan,
        "lower_tail_mean_calibrated_probability": float(lower_tail["calibrated_probability"].mean())
        if len(lower_tail)
        else np.nan,
        "lower_tail_observed_frequency": float(lower_tail["outcome"].mean())
        if len(lower_tail) and "outcome" in lower_tail.columns
        else np.nan,
    }


def build_isotonic_diagnostic_summary(diagnostics: pd.DataFrame) -> pd.DataFrame:
    rows = [isotonic_diagnostic_summary_row("overall", diagnostics)]
    for label, part in diagnostics.groupby("contract_label", sort=True):
        rows.append(isotonic_diagnostic_summary_row(str(label), part))
    return pd.DataFrame(rows)
...
def write_individual_metric_files(
    *,
    ...
    isotonic_diagnostics = build_isotonic_forecast_diagnostics(raw)
    isotonic_summary = build_isotonic_diagnostic_summary(isotonic_diagnostics)
    raw.to_csv(output_dir / "raw_values.csv", index=False)
    isotonic_diagnostics.to_csv(output_dir / "isotonic_forecast_diagnostics.csv", index=False)
    isotonic_summary.to_csv(output_dir / "isotonic_forecast_diagnostics_summary.csv", index=False)
    ...
    (output_dir / "model_m_assumptions.txt").write_text("\n".join(ASSUMPTIONS) + "\n", encoding="utf-8")
...
def svg_line_chart(
    ...
    *,
    ...
    <svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(title)}">
      ...
      {circles}
      <text x="{plot_left}" y="{height - 12}" class="axis-label">Mean Model M probability</text>
...
def svg_histogram(
    ...
    *,
    ...
    <svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(title)}">
      ...
      <line x1="{plot_left}" y1="{plot_top}" x2="{plot_left}" y2="{plot_bottom}" class="axis" />
      {''.join(bars)}
      <text x="{plot_left}" y="{height - 12}" class="axis-label">Model M probability</text>
...
def plotly_report_charts(raw: pd.DataFrame, calibration: pd.DataFrame) -> str:
    ...
    fig = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=("Calibration", "Model M Probability Distribution"),
    ...
    fig.update_xaxes(range=[0, 1], title_text="Mean Model M probability", row=1, col=1)
    fig.update_yaxes(range=[0, 1], title_text="Observed frequency", row=1, col=1)
    fig.update_xaxes(range=[0, 1], title_text="Model M probability", row=1, col=2)
    fig.update_yaxes(title_text="Count", row=1, col=2)
    fig.update_layout(template="plotly_dark", height=420, showlegend=False, margin=dict(l=40, r=30, t=70, b=40))
...
def fallback_report_charts(raw: pd.DataFrame, calibration: pd.DataFrame) -> str:
    ...
    return (
        ...
        + svg_histogram(raw, value_col="p_kalshi", title="Model M Probability Distribution")
        + "</div>"
    )
...
<body>
<main>
  <h1>Model M: Probability Evaluation</h1>
  <p>
    Forecasts are minute-level Model M YES probabilities from the hourly forecast files. Reality is the
  ...
  <h2>Output Files</h2>
  <p class="note">
    Individual CSV outputs are saved next to this report in <code>model_M_Evals_Outputs</code>:
    raw values, Brier score, Brier decomposition, log loss, calibration curve,
    expanded calibration error, sharpness, isotonic forecast diagnostics, outcome join coverage,
    unmatched rows, and audit mismatches.
  </p>
  <ul>{message_html}</ul>
</main>
</body>
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Model M Market Minute Bucket Summary</title>
  <style>
    :root {{
      color-scheme: dark;
      --bg: #07111f;
...
<body>
<main>
  <h1>Model M: Market Minute Buckets</h1>
  <p>
...
def main() -> None:
    ...
    summary_path = output_dir / "model_m_summary.html"
    ...
    summary_with_coverage_path = output_dir / "model_m_summary_with_coverage.html"
    ...
    print(f"Model M outputs saved to: {output_dir}")
