...
OUTPUT_FOLDER_NAME = "Model_C_Stats_Dashboard_Outputs"
...
def default_model_c_eval_dir(root: Path) -> Path:
    return root / "Model_C" / "model_C_Evals_Outputs"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build dark-mode internal and comparative dashboards for Model C using the "
    ...
    parser.add_argument(
        "--model-c-eval-dir",
        type=Path,
        default=default_model_c_eval_dir(REPO_ROOT),
        help="Directory containing Model_C evaluation outputs.",
    )
    parser.add_argument(
        "--model-k-eval-dir",
        type=Path,
...
def internal_charts_html(bundle: Dict[str, Any], pit: np.ndarray) -> str:
    ...
    fig_top = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=("Calibration Curve", "Model C Probability Distribution"),
    ...
    fig_top.update_xaxes(range=[0, 1], title_text="Mean Model C probability", row=1, col=1)
    fig_top.update_yaxes(range=[0, 1], title_text="Observed frequency", row=1, col=1)
    fig_top.update_xaxes(range=[0, 1], title_text="Model C probability", row=1, col=2)
...
def comparison_charts_html(bundle_a: Dict[str, Any], bundle_k: Dict[str, Any]) -> str:
    ...
    fig_bottom.add_trace(
        go.Bar(x=labels, y=[float(dec_a[c]) for c in cols], marker_color=COLOR_A, name="Model C"),
        row=1,
        col=1,
    )
    fig_bottom.add_trace(
...
<head>
  ...
  <title>Model C Internal Dashboard</title>
  {dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    ...
    <h1>Model C Probabilistic Diagnostics</h1>
    <p class="lead">
      This dashboard summarizes Model C as a probabilistic forecaster on its own scored output universe,
...
def overview_comparison_table(bundle_a: Dict[str, Any], bundle_k: Dict[str, Any]) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [
            {"metric": metric, "model_c": left, "model_k": right, "delta_c_minus_k": left - right}
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Model C vs Model K Dashboard</title>
  {dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    ...
    <h1>Model C vs Model K</h1>
    <p class="lead">
      This dashboard compares Model C and Model K on the exact overlapping scored-row universe,
    ...
    <div class="hero-pills">
      ...
      <span class="pill">Model C Brier: {format_number(oa["brier_score"])}</span>
...
def main() -> None:
    ...
    model_c_eval_dir = args.model_c_eval_dir.resolve()
    model_k_eval_dir = args.model_k_eval_dir.resolve()

    loaded_b = load_eval_dir(model_c_eval_dir)
    ...
    internal_notes = [
        f"Source directory: {model_c_eval_dir}",
        f"Randomized PIT seed: {args.pit_seed}",
        "Internal dashboard uses the full Model C scored universe currently present in raw_values.csv.",
    ]
    internal_html = build_internal_dashboard_html(bundle_a_internal, pit_values, internal_notes)
    (output_dir / "model_c_internal_dashboard.html").write_text(internal_html, encoding="utf-8")

    aligned_b_raw, aligned_k_raw = intersect_raw_rows(loaded_b["raw"], loaded_k["raw"])
    if aligned_b_raw.empty or aligned_k_raw.empty:
        raise RuntimeError("No overlapping scored rows were found between Model C and Model K.")
    ...
    comparative_notes = [
        f"Model C source directory: {model_c_eval_dir}",
    ...
    (output_dir / "model_c_vs_model_k_dashboard.html").write_text(comparative_html, encoding="utf-8")

    print(f"Internal dashboard: {output_dir / 'model_c_internal_dashboard.html'}")
    print(f"Comparative dashboard: {output_dir / 'model_c_vs_model_k_dashboard.html'}")
