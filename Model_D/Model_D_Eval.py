...
SYMBOL = "BTCUSDT"
DEFAULT_MC_TRIALS = 20_000

CONTRACT_LABELS = ("ATM", "OTM+1", "OTM-1")
EPS = 1e-15
ASSUMPTIONS = (
    "Model D forecast values are interpreted as YES probabilities. CSV values greater than 1 and no greater than 100 "
    ...
    "Monte Carlo self-metric error estimates treat each forecast probability as a finite-path binomial estimate "
    "with variance p(1-p)/N, where N is the configured --mc-trials. Brier and log-loss uncertainty use "
    "first-order propagation, and classification accuracy uses a normal threshold-crossing approximation.",
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate Model D BTC hourly contract probabilities against official "
    ...
    parser.add_argument(
        ...
        help="Folder containing Model_D.py hourly raw forecast CSV outputs.",
    )
    parser.add_argument(
        "--settlement-csv",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "model_D_Evals_Outputs",
        help="Directory for Model D evaluation output files.",
    )
    parser.add_argument(
        "--skip-binance-audit",
        action="store_true",
    ...
    parser.add_argument(
        "--mc-trials",
        type=int,
        default=DEFAULT_MC_TRIALS,
        help="Monte Carlo path count used to estimate each forecast probability for self-metric error estimates.",
    )
...
def monte_carlo_metric_estimates(frame: pd.DataFrame, threshold: float, mc_trials: int) -> Dict[str, Any]:
    if frame.empty:
        return {
            "mc_trials": int(mc_trials),
            "mean_p_kalshi_mc_se": np.nan,
            "brier_score_mc_se": np.nan,
            "log_loss_mc_se": np.nan,
            "classification_accuracy_mc_se": np.nan,
        }

    p = frame["p_kalshi"].clip(EPS, 1.0 - EPS).to_numpy(dtype=float)
    y = frame["outcome"].to_numpy(dtype=int)
    n_forecasts = float(len(frame))
    probability_variance = (p * (1.0 - p)) / float(mc_trials)

    mean_probability_se = math.sqrt(float(probability_variance.sum())) / n_forecasts

    brier_gradient = 2.0 * (p - y)
    brier_score_se = math.sqrt(float(((brier_gradient ** 2) * probability_variance).sum())) / n_forecasts

    log_loss_gradient = ((1 - y) / np.clip(1.0 - p, EPS, None)) - (y / np.clip(p, EPS, None))
    log_loss_se = math.sqrt(float(((log_loss_gradient ** 2) * probability_variance).sum())) / n_forecasts

    predicted = (p >= threshold).astype(int)
    sigma = np.sqrt(probability_variance)
    flip_probability = np.zeros_like(p)
    nonzero_sigma = sigma > 0.0
    if np.any(nonzero_sigma):
        threshold_z = (threshold - p[nonzero_sigma]) / sigma[nonzero_sigma]
        threshold_cdf = standard_normal_cdf(threshold_z)
        flip_probability[nonzero_sigma] = np.where(
            predicted[nonzero_sigma] == 1,
            threshold_cdf,
            1.0 - threshold_cdf,
        )
    flip_probability = np.clip(flip_probability, 0.0, 1.0)
    rerun_accuracy_probability = np.where(predicted == y, 1.0 - flip_probability, flip_probability)
    classification_accuracy_se = math.sqrt(
        float((rerun_accuracy_probability * (1.0 - rerun_accuracy_probability)).sum())
    ) / n_forecasts

    return {
        "mc_trials": int(mc_trials),
        "mean_p_kalshi_mc_se": mean_probability_se,
        "brier_score_mc_se": brier_score_se,
        "log_loss_mc_se": log_loss_se,
        "classification_accuracy_mc_se": classification_accuracy_se,
    }
    ...
    *,
    ...
    if frame.empty:
        ...
        if mc_trials is not None:
            row.update(monte_carlo_metric_estimates(frame, threshold, mc_trials))
        return row
...
def build_metrics_summary(
    ...
    *,
    ...
    rows = [metric_row("overall", df, threshold, mc_trials=mc_trials)]
    for label, part in df.groupby("contract_label", sort=True):
        rows.append(metric_row(str(label), part, threshold, mc_trials=mc_trials))
    return pd.DataFrame(rows)
...
def write_individual_metric_files(
    *,
    ...
    (output_dir / "model_d_assumptions.txt").write_text("\n".join(ASSUMPTIONS) + "\n", encoding="utf-8")
...
def svg_line_chart(
    ...
    *,
    ...
    <svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(title)}">
      ...
      {circles}
      <text x="{plot_left}" y="{height - 12}" class="axis-label">Mean Model D probability</text>
...
def svg_histogram(
    ...
    *,
    ...
    <svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(title)}">
      ...
      <line x1="{plot_left}" y1="{plot_top}" x2="{plot_left}" y2="{plot_bottom}" class="axis" />
      {''.join(bars)}
      <text x="{plot_left}" y="{height - 12}" class="axis-label">Model D probability</text>
...
def plotly_report_charts(raw: pd.DataFrame, calibration: pd.DataFrame) -> str:
    ...
    fig = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=("Calibration", "Model D Probability Distribution"),
    ...
    fig.update_xaxes(range=[0, 1], title_text="Mean Model D probability", row=1, col=1)
    fig.update_yaxes(range=[0, 1], title_text="Observed frequency", row=1, col=1)
    fig.update_xaxes(range=[0, 1], title_text="Model D probability", row=1, col=2)
...
def fallback_report_charts(raw: pd.DataFrame, calibration: pd.DataFrame) -> str:
    ...
    return (
        ...
        + svg_histogram(raw, value_col="p_kalshi", title="Model D Probability Distribution")
        + "</div>"
    )
...
def build_summary_html(
    *,
    ...
    if "brier_score_mc_se" in overall.index:
        cards.extend(
            [
                ("Brier MC SE", fmt_num(overall["brier_score_mc_se"])),
                ("Log loss MC SE", fmt_num(overall["log_loss_mc_se"])),
            ]
        )
    card_html = "\n".join(
...
<body>
<main>
  <h1>Model D: Probability Evaluation</h1>
  <p>
    Forecasts are minute-level Model D YES probabilities from the hourly forecast files. Reality is the
  ...
  <h2>Output Files</h2>
  <p class="note">
    Individual CSV outputs are saved next to this report in <code>model_D_Evals_Outputs</code>:
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Model D Market Minute Bucket Summary</title>
  <style>
    :root {{
      color-scheme: dark;
      --bg: #07111f;
...
<body>
<main>
  <h1>Model D: Market Minute Buckets</h1>
  <p>
...
def main() -> None:
    ...
    metrics = build_metrics_summary(raw, threshold=args.classification_threshold, mc_trials=args.mc_trials)
    ...
    summary_path = output_dir / "model_d_summary.html"
    ...
    summary_with_coverage_path = output_dir / "model_d_summary_with_coverage.html"
    ...
    print(f"Model D outputs saved to: {output_dir}")
