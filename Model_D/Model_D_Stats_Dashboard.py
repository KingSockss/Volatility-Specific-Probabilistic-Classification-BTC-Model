...
def load_model_d_eval_module() -> Any:
    module_path = Path(__file__).resolve().with_name("Model_D_Eval.py")
    spec = importlib.util.spec_from_file_location("model_d_eval_for_stats_dashboard", module_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not load Model_D_Eval.py from {module_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


_MODEL_D_EVAL = load_model_d_eval_module()
DEFAULT_MC_TRIALS = _MODEL_D_EVAL.DEFAULT_MC_TRIALS
build_metrics_summary = _MODEL_D_EVAL.build_metrics_summary
...
OUTPUT_FOLDER_NAME = "Model_D_Stats_Dashboard_Outputs"
...
def default_model_d_eval_dir(root: Path) -> Path:
    return root / "Model_D" / "model_D_Evals_Outputs"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build dark-mode internal and comparative dashboards for Model D using the "
    ...
    parser.add_argument(
        "--model-d-eval-dir",
        type=Path,
        default=default_model_d_eval_dir(REPO_ROOT),
        help="Directory containing Model_D evaluation outputs.",
    )
    parser.add_argument(
        "--model-k-eval-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Monte Carlo path count used to estimate Model D self-metric error bars in the internal dashboard.",
    )
    parser.add_argument(
        "--pit-seed",
        type=int,
...
def build_bundle_from_raw(
    ...
    *,
    ...
    metrics = build_metrics_summary(raw, threshold=classification_threshold, mc_trials=mc_trials)
...
def internal_charts_html(bundle: Dict[str, Any], pit: np.ndarray) -> str:
    ...
    fig_top = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=("Calibration Curve", "Model D Probability Distribution"),
    ...
    fig_top.update_xaxes(range=[0, 1], title_text="Mean Model D probability", row=1, col=1)
    fig_top.update_yaxes(range=[0, 1], title_text="Observed frequency", row=1, col=1)
    fig_top.update_xaxes(range=[0, 1], title_text="Model D probability", row=1, col=2)
...
def comparison_charts_html(bundle_a: Dict[str, Any], bundle_k: Dict[str, Any]) -> str:
    ...
    fig_bottom.add_trace(
        go.Bar(x=labels, y=[float(dec_a[c]) for c in cols], marker_color=COLOR_A, name="Model D"),
        row=1,
        col=1,
    )
    fig_bottom.add_trace(
...
def build_internal_dashboard_html(bundle: Dict[str, Any], pit: np.ndarray, notes: Sequence[str]) -> str:
    ...
    if "brier_score_mc_se" in overall.index:
        cards.extend(
            [
                ("Brier MC SE", format_number(overall["brier_score_mc_se"])),
                ("Log loss MC SE", format_number(overall["log_loss_mc_se"])),
            ]
        )
    card_html = "".join(
...
<head>
  ...
  <title>Model D Internal Dashboard</title>
  {dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    <p class="eyebrow">Internal Model Evaluation</p>
    <h1>Model D Probabilistic Diagnostics</h1>
    <p class="lead">
      This dashboard summarizes Model D as a probabilistic forecaster on its own scored output universe,
...
def overview_comparison_table(bundle_a: Dict[str, Any], bundle_k: Dict[str, Any]) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [
            {"metric": metric, "model_d": left, "model_k": right, "delta_c_minus_k": left - right}
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Model D vs Model K Dashboard</title>
  {dashboard_style()}
</head>
<body>
<main>
  <section class="hero">
    <p class="eyebrow">Comparative Model Evaluation</p>
    <h1>Model D vs Model K</h1>
    <p class="lead">
      This dashboard compares Model D and Model K on the exact overlapping scored-row universe,
    ...
    <div class="hero-pills">
      ...
      <span class="pill">Model D Brier: {format_number(oa["brier_score"])}</span>
...
def main() -> None:
    args = parse_args()
    if args.model_d_mc_trials <= 1:
        raise ValueError("--model-d-mc-trials must be greater than 1.")

    output_dir = args.output_dir.resolve()
    output_dir.mkdir(parents=True, exist_ok=True)

    model_d_eval_dir = args.model_d_eval_dir.resolve()
    model_k_eval_dir = args.model_k_eval_dir.resolve()

    loaded_b = load_eval_dir(model_d_eval_dir)
    ...
    bundle_a_internal = build_bundle_from_raw(
        ...
        mc_trials=args.model_d_mc_trials,
    ...
    internal_notes = [
        f"Source directory: {model_d_eval_dir}",
        f"Randomized PIT seed: {args.pit_seed}",
        "Internal dashboard uses the full Model D scored universe currently present in raw_values.csv.",
    ]
    internal_html = build_internal_dashboard_html(bundle_a_internal, pit_values, internal_notes)
    (output_dir / "model_d_internal_dashboard.html").write_text(internal_html, encoding="utf-8")

    aligned_b_raw, aligned_k_raw = intersect_raw_rows(loaded_b["raw"], loaded_k["raw"])
    if aligned_b_raw.empty or aligned_k_raw.empty:
        raise RuntimeError("No overlapping scored rows were found between Model D and Model K.")
    ...
    comparative_notes = [
        f"Model D source directory: {model_d_eval_dir}",
    ...
    (output_dir / "model_d_vs_model_k_dashboard.html").write_text(comparative_html, encoding="utf-8")

    print(f"Internal dashboard: {output_dir / 'model_d_internal_dashboard.html'}")
    print(f"Comparative dashboard: {output_dir / 'model_d_vs_model_k_dashboard.html'}")
