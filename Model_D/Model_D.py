...
USER_AGENT = "model-d-high-vol-garch-studentt-builder/1.0"

OUTPUT_FOLDER_NAME = "Model_D_Output_Raw_Data"
EPS = 1e-12
MIN_VARIANCE = 1e-12
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build Model D raw hourly forecast files by combining settlement-defined Kalshi "
    ...
    parser.add_argument(
        ...
        type=int,
        default=240,
        help="Minimum number of regime-filtered 1-minute returns required before a Model D fit is allowed.",
    )
    ...
    for schedule_idx, forecast in enumerate(schedule.to_dict(orient="records"), start=1):
        ...
        if need_refit:
            ...
            if regime_sample.n_returns < args.min_regime_observations:
                raise RuntimeError(
                    "Insufficient regime-filtered history for Model D fit at "
