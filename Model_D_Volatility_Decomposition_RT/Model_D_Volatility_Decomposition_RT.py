...
from Model_D.Model_D_Eval import (  # noqa: E402
    ASSUMPTIONS,
    DEFAULT_MC_TRIALS,
    attach_outcomes,
...
def default_binance_cache_csv(root: Path) -> Path | None:
    return first_existing(
        [
            root / "Model_K_Volatility_Decomposition_RT" / "Model_K_Volatility_Decomposition_RT_outputs" / "binance_1m_klines.csv",
            root / "Model_K_Volatility_Results_Dashboard" / "Model_K_Volatility_Dashboards_outputs" / "binance_1m_klines.csv",
        ]
    )


def default_hourly_states_csv(root: Path) -> Path | None:
    return first_existing(
        [
            root / "Model_K_Volatility_Decomposition_RT" / "Model_K_Volatility_Decomposition_RT_outputs" / "hourly_market_volatility_segments.csv",
            root / "Model_D_Volatility_Decomposition_RT" / "Model_D_Volatility_Decomposition_RT_outputs" / "hourly_market_volatility_segments.csv",
        ]
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run Model D outputs separately across hourly Kalshi BTC market states "
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "Model_D_Volatility_Decomposition_RT_outputs",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "If present and it covers the filtered Model D event universe, it is reused."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Monte Carlo path count used to estimate Model D self-metric error bars in each volatility segment.",
    )
...
def retitle_summary_html(document: str, *, heading: str, output_folder_name: str) -> str:
    updated = document.replace("<title>Model D Summary</title>", f"<title>{html.escape(heading)}</title>")
...
def build_segment_outputs(
    *,
    ...
    metrics = build_metrics_summary(raw_segment, threshold=classification_threshold, mc_trials=mc_trials)
    ...
    (segment_dir / "model_d_assumptions.txt").write_text("\n".join(all_assumptions) + "\n", encoding="utf-8")
    ...
    summary_heading = f"Model D RT: {segment_name}"
    ...
    (segment_dir / "model_d_summary.html").write_text(
    ...
    (segment_dir / "model_d_summary_with_coverage.html").write_text(
        retitle_summary_html(
            summary_with_coverage_html,
            heading=f"Model D RT: {segment_name} With Coverage",
    ...
    (segment_dir / "time_bucket_summary.html").write_text(
        retitle_time_bucket_html(
            time_bucket_html,
            heading=f"Model D RT: {segment_name} Market Minute Buckets",
        ),
        encoding="utf-8",
    )
...
def build_index_html(
    *,
    ...
    for row in segment_rows.to_dict(orient="records"):
        ...
        summary_href = f"{segment_name}/model_d_summary.html"
        summary_with_coverage_href = f"{segment_name}/model_d_summary_with_coverage.html"
...
<head>
  ...
  <title>Model D Real-Time Volatility Decomposition</title>
  {dark_mode_style_block()}
</head>
<body>
<main>
  <h1>Model D Real-Time Volatility Decomposition</h1>
  <p>
    This directory contains five segment-specific copies of the usual Model D outputs, split by
  ...
  <ul>
    ...
    <li><code>all_scored_rows_with_volatility.csv</code>: scored Model D rows with attached hourly volatility metadata.</li>
...
def main() -> None:
    ...
    preferred_states_path = args.hourly_market_states_csv.resolve() if args.hourly_market_states_csv else None
    preferred_hourly_states = None
    if preferred_states_path and preferred_states_path.exists():
        ...
        forecasts = forecasts[forecasts["event_ticker"].astype(str).isin(allowed_events)].copy()
    ...
    if preferred_hourly_states is not None:
        required_events = set(raw["event_ticker"].astype(str))
        available_events = set(preferred_hourly_states["event_ticker"].astype(str))
        missing_events = sorted(required_events - available_events)
        if missing_events:
            sample = ", ".join(missing_events[:5])
            raise ValueError(
                "Precomputed hourly volatility table does not cover the filtered Model D event universe. "
                f"Example missing event_ticker(s): {sample}"
            )
        hourly_market_states = preferred_hourly_states[
            preferred_hourly_states["event_ticker"].astype(str).isin(required_events)
        ].copy()
        thresholds = thresholds_table(hourly_market_states)
        hourly_volatility = pd.DataFrame()
        sibling_hourly_vol = preferred_states_path.with_name("binance_hourly_realized_volatility.csv")
        if sibling_hourly_vol.exists():
            shutil.copy2(sibling_hourly_vol, output_root / "binance_hourly_realized_volatility.csv")
        if args.binance_minute_cache_csv:
            preferred_cache_path = args.binance_minute_cache_csv.resolve()
            if preferred_cache_path.exists():
                shutil.copy2(preferred_cache_path, output_root / "binance_1m_klines.csv")
    else:
    ...
    if not hourly_volatility.empty:
    ...
    segment_rows["model_d_summary_html"] = segment_rows["segment_name"].map(
        lambda value: f"{value}/model_d_summary.html"
    )
    segment_rows["model_d_summary_with_coverage_html"] = segment_rows["segment_name"].map(
        lambda value: f"{value}/model_d_summary_with_coverage.html"
    )
    ...
    print(f"Model D real-time volatility decomposition outputs saved to: {output_root}")
